...
namespace fTransport
{
    public class Transportables
    {
        ...
        public void IniciarTablaListPlus<T>(DataGridView dg, List<T> list, List<DataGridField> aCampos, bool Encabezado = true,
            bool EditaCampos = false, Font fnt = null, bool EncabezadoDeFila = true, int AltoDeFila = 18,
            bool AsignaDataSource = true, bool SeleccionSimple = true, DataGridViewColumn lastCol = null)
        {
            ...
            foreach (DataGridField campo in aCampos)
            {
                ...
                using (DataGridViewColumn dgCol = new DataGridViewColumn())
                {
                    ...
                    dgCol.Name = campo.Campo;
                    dgCol.HeaderText = campo.Nombre;
                    dgCol.DataPropertyName = campo.Campo;
                    dgCol.Width = campo.Ancho;
                    dgCol.Resizable = DataGridViewTriState.False;
                    dgCol.HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter;
                    ...
                    dgCol.SortMode = DataGridViewColumnSortMode.NotSortable;
                    dgCol.DefaultCellStyle.Alignment = campo.Alineacion;
                    dgCol.DefaultCellStyle.Format = campo.Formato;

                    if (campo == aCampos.Last()) { dgCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill; }
            ...
            bool AsignaDataSource = true, bool SeleccionSimple = true, bool UnidadDeSeleccion = false)
        {
            dg.Columns.Clear();
            dg.AutoGenerateColumns = false;
            dg.SelectionMode = GridSelectionMode.Single;
            dg.Style.SelectionStyle.BackColor = Color.DodgerBlue;
            dg.Style.SelectionStyle.TextColor = Color.White;
            dg.Style.RowHeaderStyle.BackColor = Color.DodgerBlue;
            dg.Style.RowHeaderStyle.SelectionBackColor = Color.DodgerBlue;
            dg.Style.RowHeaderStyle.SelectionMarkerColor = Color.White;
            dg.BackColor = Color.Gainsboro;


            //dg .AlternatingRowsDefaultCellStyle.BackColor = System.Drawing.Color.AliceBlue;
            if (fnt == null)
            {
                fnt = new Font("Consolas", 9, FontStyle.Regular);

            };
            Font headerFont = new Font(fnt.FontFamily, fnt.Size, FontStyle.Bold);

            dg.Style.CellStyle.Font = new GridFontInfo(fnt);
            dg.Style.HeaderStyle.Font = new GridFontInfo(headerFont);
            dg.ShowRowHeader = EncabezadoDeFila;
            ...
            dg.SearchController.AllowFiltering = true;
            // dg.AllowUserToAddRows = false;

            dg.SelectionUnit = UnidadDeSeleccion
                ? SelectionUnit.Cell
                : SelectionUnit.Row;


            if (EditaCampos) dg.EditMode = EditMode.DoubleClick;
            ...
            {
                dg.SelectionMode = GridSelectionMode.Multiple;
                dg.Columns.Add(new GridCheckBoxSelectorColumn()
                {
            ...
            foreach (SfDataGridField field in fields)
            {
                switch (field.Type)
                {
                    ...
                    case TypeColumn.DateTimeColumn:
                        dg.Columns.Add(new GridDateTimeColumn()
                        {
                            MappingName = field.Campo,
                            HeaderText = field.Nombre,
                            AllowNull = true,
                        ...
                        break;
                    case TypeColumn.NumericColumn:
                        dg.Columns.Add(new GridNumericColumn()
                        {
                            MappingName = field.Campo,
                            HeaderText = field.Nombre,
                            AllowNull = true,
                ...
                {
                    // dg.Columns[field.Campo].AutoSizeColumnsMode = AutoSizeColumnsMode.LastColumnFill;
                }
                else
                {
                    dg.Columns[field.Campo].AllowSorting = false;
                    dg.Columns[field.Campo].AllowEditing = false;
                    dg.Columns[field.Campo].CellStyle.HorizontalAlignment = field.Alineacion;
        ...
        public string ArchivoEnBaseDatos_A_ArchivoEnDisco(System.Data.DataRow MyRow, string Campo, string Nombre, string extension)
        {
            string Camino = "";
