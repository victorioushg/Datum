...
Module FuncionesImpresionFiscalPnP
    ...
    Public Sub ImprimirFacturaFiscalPnP(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal NumeroFactura As String, ByVal NombreCliente As String, _
       ...
       ByVal CondicionPago As String, ByVal FechaVencimiento As Date, ByVal CodigoCajero As String, ByVal NombreCajero As String)
        ...
        ' Verifica impresora
        If Not AbrirPuertoPnP(MyConn, lblInfo) Then Return
        ...
        If dtRenglones.Rows.Count > 0 Then
            For bbCont = 0 To dtRenglones.Rows.Count - 1
                With dtRenglones.Rows(bbCont)
                    Dim Descripcion As String = .Item("UNIDAD") & " " & .Item("DESCRIP")
                    bRet = ""
                    While bRet <> "OK"
                        PFrenglon(Descripcion, FormatoCantidadFiscalEpson(.Item("CANTIDAD")), FormatoPrecioFiscalEpson(.Item("TOTRENDES") / .Item("CANTIDAD")), IVAFiscalEpson(MyConn, lblInfo, FechaEmision, .Item("IVA")))
                    End While
                End With
            Next
        End If
        ...
        ds = DataSetRequery(ds, " select * from jsvenforpag where numfac = '" & NumeroFactura & "' and origen = 'FAC' and id_emp = '" & jytsistema.WorkID & "' ", MyConn, nTablaFP, lblInfo)
        ...
        ActualizaNumeroControl(False, MyConn, lblInfo, NumeroFactura, FechaIvaFactura, "FAC", "FAC", CodigoCliente)
    ...
    Public Sub ImprimirNotaCreditoPnP(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal NumeroNotaCredito As String, _
        ...
        Dim dtRenglones As DataTable
        Dim nTablaRenglon As String = "tblrenglones"
        ds = DataSetRequery(ds, " select * from jsvenrenncr where numfac = '" & NumeroNotaCredito & "' and id_emp = '" & jytsistema.WorkID & "' ", MyConn, nTablaRenglon, lblInfo)
        dtRenglones = ds.Tables(nTablaRenglon)
        If dtRenglones.Rows.Count > 0 Then
        ...
        '//// ACTUALIZAR BIT IMPRESION EN NOTA CREDITO
        ft.Ejecutar_strSQL(myconn, " update jsvenencncr set impresa = '1' where numncr = '" & NumeroNotaCredito & "' and id_emp = '" & jytsistema.WorkID & "' ")
        '//// ACTUALIZAR NUMERO DE CONTROL"
        Dim FechaIvaFactura As Date = ft.DevuelveScalarFecha(MyConn, "select emision from jsvenencncr where numfac = '" & NumeroNotaCredito & "' and id_emp = '" & jytsistema.WorkID & "' ").ToString
        ActualizaNumeroControl(False, MyConn, lblInfo, NumeroNotaCredito, FechaIvaFactura, "NCR", "FAC", Cliente)

    End Sub

    Public Sub ImprimirNotaDebitoFiscalPnP(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal NumeroNotaDebito As String, ByVal NombreCliente As String, _
       ByVal CodigoCliente As String, ByVal RIF As String, ByVal DireccionFiscal As String, ByVal FechaEmision As Date, _
       ByVal CondicionPago As String, ByVal FechaVencimiento As Date, ByVal CodigoCajero As String, ByVal NombreCajero As String)
        ...
        'Imprimir Encabezado 
        ImprimeEncabezadoFacturaPnP(NombreCliente, NumeroNotaDebito, FechaEmision, DireccionFiscal, RIF, CondicionPago, FechaVencimiento)
        ...
        ActualizaNumeroControl(False, MyConn, lblInfo, NumeroNotaDebito, FechaIvaFactura, "NDB", "FAC", CodigoCliente)
    ...
    Private Sub ImprimeEncabezadoFacturaPnP(ByVal Cliente As String, ByVal NumeroFactura As String, ByVal FechaEmision As Date, _
        ...
        bRet = ""
        While bRet <> "OK"
            bRet = PFTfiscal(IIf(CondPago = CondicionPago.iCredito, "CREDITO, VENCE: " & ft.FormatoFecha(FechaVencimiento), "CONTADO"))
        End While
        bRet = ""
        While bRet <> "OK"
    ...
    Private Sub ImprimeEncabezadoDevolucionPnP(ByVal Cliente As String, ByVal NumeroFacturaAfectada As String, ByVal FechaEmision As Date, _
        ...
        bRet = ""
        While bRet <> "OK"
            PFDevolucion(Cliente, RIF, NumeroFacturaAfectada, SerialMáquinaFacturaAfectada, FechaFacturaAfectada, HoraFacturaAfectada)
        End While
