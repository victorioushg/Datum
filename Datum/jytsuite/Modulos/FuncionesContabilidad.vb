...
Module FuncionesContabilidad
    ...
    Public Function ValidarEstructuraActual(ByVal Myconn As MySqlConnection, ByVal lblInfo As Label, ByVal CadenaPrueb As String, ByVal EstructuraPrueb() As Integer, ByVal LongNivelesPrueb() As Integer) As Boolean
        ...
        If EstructuraValidaPlus(Myconn, CadenaPrueb) Then
        ...
        End If
    End Function
    Public Function EstructuraValidaPlus(MyConn As MySqlConnection, CadenaAValidar As String) As Boolean

        Dim Maskara As String = Convert.ToString(ParametroPlus(MyConn, Gestion.iContabilidad, "CONPARAM01"))
        If Maskara.Trim() <> "" Then
            Dim eMask() As String = Maskara.Split(".")
            Dim eCadC() As String = CadenaAValidar.Split(".")

            'VALIDA CUENTA RESULTADO
            If eCadC.Length < eMask.Length And Not CadenaAValidar.EndsWith(".") Then Return False

            Dim nCadena As String = ""
            For iCont As Integer = 0 To eCadC.Length - 1
                'VALIDA LONGITUDES DE TOKENS
                If eCadC(iCont).Length <> eMask(iCont).Length Then Return False
                If iCont = eMask.Length - 1 Then
                    nCadena += eCadC(iCont)
                Else
                    nCadena += eCadC(iCont) + "."
                End If
                If nCadena = CadenaAValidar Then Return True
            Next
    ...
    Public Sub ActualizaSaldoCuenta(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoCuenta As String, ByVal Fecha As Date)
        ...
        If Fecha < FechaInicioEjercicio(MyConn, lblInfo) Then
            strFecha = " b.fechasi < '" & ft.FormatoFechaMySQL(FechaInicioEjercicio(MyConn, lblInfo)) & "' and "
            strMes = "00"
        Else
        ...
        'ft.Ejecutar_strSQL(MyConn, " update jscotdaacon a, (select substring(a.codcon,1," & Len(CodigoCuenta) & ") codcon, " _
        '        & " sum( if( a.debito_credito = 0 , a.importe, 0) ) debitos, " _
        ...
        Dim bDebitos As Double = ft.DevuelveScalarDoble(MyConn, " select sum( if( a.debito_credito = 0 , a.importe, 0) )  " _
                                        & " from jscotrenasi a " _
                                        & " Left join jscotencasi b on (a.asiento = b.asiento and a.actual = b.actual and a.ejercicio = b.ejercicio And a.id_emp = b.id_emp) " _
                                        & " Where " _
                                        & " substring(a.codcon,1," & CodigoCuenta.Length & ") = '" & CodigoCuenta & "' and " _
                                        & " b.actual = 1 and " _
                                        & strFecha _
                                        & " a.ejercicio = '" & jytsistema.WorkExercise & "' and " _
                                        & " a.id_emp = '" & jytsistema.WorkID & "' " _
                                        & " group by substring(a.codcon,1," & CodigoCuenta.Length & ") ")

        Dim bCreditos As Double = ft.DevuelveScalarDoble(MyConn, " select sum( if( a.debito_credito = 1 , a.importe, 0) ) " _
                                        & " from jscotrenasi a " _
                                        & " Left join jscotencasi b on (a.asiento = b.asiento and a.actual = b.actual and a.ejercicio = b.ejercicio And a.id_emp = b.id_emp) " _
                                        & " Where " _
                                        & " substring(a.codcon,1," & CodigoCuenta.Length & ") = '" & CodigoCuenta & "' and " _
        ...
        ft.Ejecutar_strSQL(MyConn, " update jscotdaacon a " _
                & " set a.deb" & strMes & " = " & bDebitos & ", a.cre" & strMes & " = " & bCreditos & " " _
                & " Where " _
                & " a.codcon = '" & CodigoCuenta & "' and " _
                & " a.ejercicio = '" & jytsistema.WorkExercise & "' and " _
                & " a.id_emp = '" & jytsistema.WorkID & "' ")
    ...
    Public Sub Contabilizar_Plantilla(ByVal MyConn As MySqlConnection, lblInfo As Label, ByVal ds As DataSet,
        ...
        Dim strFecha As String = "'" & ft.FormatoFechaMySQL(Fecha) & "'"
        Dim strEmpresa As String = "'" & jytsistema.WorkID & "'"
        Dim strFechaDesde As String = "'" & ft.FormatoFechaMySQL(FechaInicial) & "'"
        Dim strFechaHasta As String = "'" & ft.FormatoFechaMySQL(FechaFinal) & "'"
        ...
        If dtPlantilla.Rows.Count > 0 Then
            ...
            For fCont = 0 To dtPlantilla.Rows.Count - 1
                With dtPlantilla.Rows(fCont)
                    ...
                    dtRenglonAsiento = ft.AbrirDataTable(ds, "tblRenglonAsiento" + ft.NumeroAleatorio(1000000).ToString, MyConn, strConsul)
                    ...
                    If dtRenglonAsiento.Rows.Count > 0 Then
                        For gCont As Integer = 0 To dtRenglonAsiento.Rows.Count - 1
                            ...
                            If Monto > 0 Then
                                ...
                                InsertEditCONTABRenglonAsiento(MyConn, lblInfo, InsertarRenglon, NumeroAsiento, nRenglon, xCodcon, xRefer,
        ...
        Dim Debitos As Double = ft.DevuelveScalarDoble(MyConn, " select sum(importe) from jscotrenasi where importe > 0 and asiento = '" & NumeroAsiento & "' and id_emp = '" & jytsistema.WorkID & "' group by asiento ")
        Dim Creditos As Double = ft.DevuelveScalarDoble(MyConn, " select sum(importe) from jscotrenasi where importe <= 0 and asiento = '" & NumeroAsiento & "' and id_emp = '" & jytsistema.WorkID & "' group by asiento ")
