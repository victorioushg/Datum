...
Module TablasCompras
    ...
    Public Sub InsertEditCOMPRASProveedores(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal Insertar As Boolean, _
    ...
    ByVal Zona As String, ByVal TipoGasto As Integer, ByVal Impresa As String, ByVal NumeroGastoAnterior As String, ByVal CodigoProveedorAnterior As String)
        ...
        strSQL += ModificarCadena(OtroProveedor, "OTRO_PRO")
        strSQL += ModificarCadena(Zona, "ZONA")
        strSQL += ModificarEntero(TipoGasto, "TIPOGASTO")
        strSQL += ModificarCadena(Impresa, "IMPRESA")
        strSQL += ModificarCadena(jytsistema.WorkExercise, "EJERCICIO")
        strSQL += ModificarCadena(jytsistema.WorkID, "id_emp")
    ...
    Public Sub InsertEditCOMPRASEncabezadoPROGRAMACION(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal Insertar As Boolean,
                                                  ByVal NumeroProgramacion As String, ByVal FechaProgramacion As Date, ByVal FechaDesde As Date, _
                                                  ByVal FechaHasta As Date, ByVal Estatus As Int32, ByVal Banco As String, ByVal Comentario As String, _
                                                  ByVal TotalProgramacion As Double)
        ...
        If Insertar Then
            strSQLInicio = " insert into jsproencprg SET "
            strSQL = ""
            strSQLFin = " "
        Else
            strSQLInicio = " UPDATE jsproencprg SET "
            strSQL = ""
            strSQLFin = " WHERE " _
                & " numprg = '" & NumeroProgramacion & "' and " _
                & " id_emp = '" & jytsistema.WorkID & "' "
        End If

        strSQL += ModificarCadena(NumeroProgramacion, "NUMPRG")
        strSQL += ModificarFecha(FechaProgramacion, "FECHAPRG")
        strSQL += ModificarFecha(FechaDesde, "FECHAPRGDESDE")
        strSQL += ModificarFecha(FechaHasta, "FECHAPRGHASTA")
        strSQL += ModificarEntero(Estatus, "ESTATUSPRG")
        strSQL += ModificarCadena(Banco, "BANCOPRG")
        strSQL += ModificarCadena(Comentario, "COMEN")
        strSQL += ModificarDoble(TotalProgramacion, "TOTALPRG")
        strSQL += ModificarCadena(jytsistema.WorkID, "ID_EMP")
    ...
    End Sub
    '//////////////////////// RENGLONES
    Public Sub InsertEditCOMPRASRenglonPROGRAMACION(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal Insertar As Boolean,
                                                  ByVal NumeroProgramacion As String, ByVal CodigoProveedor As String, _
                                                  ByVal NumeroDocumento As String, ByVal TipoDocumento As String, ByVal Emision As Date, _
                                                  ByVal Vence As Date, ByVal Importe As Double, ByVal Saldo As Double, ByVal EmisionCheque As Date, _
                                                  ByVal Banco As String, ByVal ACancelar As Double)
        ...
        If Insertar Then
            strSQLInicio = " insert into jsprorenprg SET "
            strSQL = ""
            strSQLFin = " "
        Else
            strSQLInicio = " UPDATE jsprorenprg SET "
            strSQL = ""
            strSQLFin = " WHERE " _
                & " numprg = '" & NumeroProgramacion & "' and " _
        ...
        strSQL += ModificarCadena(NumeroProgramacion, "NUMPRG")
        strSQL += ModificarCadena(CodigoProveedor, "CODPRO")
        strSQL += ModificarCadena(NumeroDocumento, "NUMMOV")
        strSQL += ModificarCadena(TipoDocumento, "TIPOMOV")
        strSQL += ModificarFecha(Emision, "EMISION")
        strSQL += ModificarFecha(Vence, "VENCE")
        strSQL += ModificarDoble(Importe, "IMPORTE")
        strSQL += ModificarDoble(Saldo, "SALDO")
        strSQL += ModificarFecha(EmisionCheque, "EMISIONCH")
        strSQL += ModificarCadena(Banco, "BANCO")
        strSQL += ModificarDoble(ACancelar, "A_CANCELAR")
        strSQL += ModificarCadena(jytsistema.WorkID, "id_emp")
