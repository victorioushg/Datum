...
Module FuncionesMercancias
    ...
    Public Function BonificacionVigenteOferta(MyConn As MySqlConnection, lblInfo As Label, dFechaActual As Date, _
        ...
        sCantidadenRenglones = CDbl(EjecutarSTRSQL_Scalar(MyConn, lblInfo, "  select " _
            & " SUM(IF( ISNULL(b.UVALENCIA), a.CANTIDAD, a.CANTIDAD/b.EQUIVALE)) AS CANTIDAD from " _
        ...
        ds = DataSetRequery(ds, " select jsmerencofe.*, jsmerrenbon.* from jsmerencofe, jsmerrenbon " _
                        ...
                        & " jsmerencofe.DESDE <= '" & FormatoFechaMySQL(dFechaActual) & "' AND " _
                        & " jsmerencofe.HASTA >= '" & FormatoFechaMySQL(dFechaActual) & "' AND " _
    ...
    Function ExistenciaEnAlmacen(ByVal MyConn As MySqlConnection, ByVal CodigoMercancia As String, _
                                  ByVal CodigoAlmacen As String, ByVal lblInfo As Label) As Double

        ExistenciaEnAlmacen = CDbl(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " SELECT SUM(IF( a.tipomov IN ('AE','EN'), 1,  " _
                                                         & " IF( a.tipomov IN ('SA', 'AS') , -1, 0 ) ) * IF( b.uvalencia IS NULL,  a.cantidad, a.cantidad/b.equivale )  ) EXISTENCIA " _
                                                         ...
                                                         & " a.fechamov <= '" & FormatoFechaHoraMySQLFinal(jytsistema.sFechadeTrabajo) & "'  AND " _
                                                         & " a.id_emp = '" & jytsistema.WorkID & "' " _
                                                         & " GROUP BY a.codart "))
    ...
    Function MercanciaPoseeMovimientos(ByVal myconn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoArticulo As String) As Boolean
        MercanciaPoseeMovimientos = CBool(EjecutarSTRSQL_Scalar(myconn, lblInfo, "SELECT COUNT(DISTINCT codart) FROM jsmertramer WHERE codart = '" & CodigoArticulo & "' AND id_emp = '" & jytsistema.WorkID & "' ").ToString)
    End Function
    Function MercanciaRegulada(ByVal myconn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoArticulo As String) As Boolean
        MercanciaRegulada = CBool(EjecutarSTRSQL_Scalar(myconn, lblInfo, "SELECT REGULADO FROM jsmerctainv WHERE codart = '" & CodigoArticulo & "' AND id_emp = '" & jytsistema.WorkID & "' ").ToString)
    End Function

    Function EsMercanciaCombo(MyConn As MySqlConnection, codigoArticulo As String) As Boolean
        Dim cantidadCombo As Integer = CInt(EjecutarSTRSQL_ScalarPLUS(MyConn, " select count(*) cuenta from jsmercatcom " _
                                                                      & " where " _
    ...
    Function PrecioOferta(ByVal MyConn As MySqlConnection, ByVal CodigoMercancia As String, _
        ...
        ds = DataSetRequery(ds, "SELECT b.porcentaje FROM jsmerencofe a  " _
                                    ...
                                    & " WHERE " _
                                    & " a.id_emp = '" & jytsistema.WorkID & "' " _
                                    & " AND a.desde <= '" & FormatoFechaMySQL(Fecha) & "' " _
                                    & " AND a.hasta >= '" & FormatoFechaMySQL(Fecha) & "' " _
    ...
    Function MensajeOferta(ByVal MyConn As MySqlConnection, ByVal CodigoArticulo As String, ByVal Fecha As Date, _
        ...
        If dt.Rows.Count > 0 Then
            With dt.Rows(0)
                MensajeOferta = " Oferta N°: " & .Item("codofe") & " Válida desde : " & FormatoFecha(CDate(.Item("desde").ToString)) & " - Hasta : " & FormatoFecha(CDate(.Item("hasta").ToString))
                If .Item("porcentajedescuento") > 0 Then _
                    MensajeOferta = MensajeOferta & vbCrLf & vbTab & "De " & FormatoCantidad(.Item("de")) & " a " & FormatoCantidad(.Item("a")) & " " & .Item("unidad") & "  " & .Item("otorgadopor") & " otorgará " & .Item("porcentajedescuento") & "% de descuento "
            End With
        End If
        ...
        If dt.Rows.Count > 0 Then
            With dt.Rows(0)
                MensajeOferta = MensajeOferta & vbCrLf & vbTab & "A partir de " & FormatoCantidad(.Item("a_partir")) & " " & .Item("unidad") & " " & .Item("otorgadapor") & " bonificará " & FormatoCantidad(.Item("se_bonificara")) & _
                    " " & .Item("unidadbon") & " de la mercancía " & .Item("de") & " cada " & FormatoCantidad(.Item("porcada")) & " " & .Item("unidad") & " adicional "
            End With
        End If
        ...
        ds = Nothing
    End Function
    Public Sub ActualizarExistenciasPlus(ByVal MyConn As MySqlConnection, ByVal CodigoMercancia As String)

        Dim lblInfo As New Label

        Dim existenciaActual As Double = CDbl(EjecutarSTRSQL_ScalarPLUS(MyConn, " SELECT SUM(IF( a.tipomov IN ('AE','EN','DV'), 1,  IF( a.tipomov IN ('SA', 'AS','DC') , -1, 0 ) ) * IF( b.uvalencia IS NULL,  a.cantidad, a.cantidad/b.equivale )  ) EXISTENCIA  " _
                                                                   & " FROM jsmertramer a  " _
                                                                   & " LEFT JOIN jsmerequmer b ON (a.codart = b.codart AND a.unidad = b.uvalencia AND a.id_emp = b.id_emp) " _
                                                                   & " WHERE " _
                                                                   & " a.codart  = '" & CodigoMercancia & "' AND  " _
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " update jsmerctainv set existe_act = " & existenciaActual.ToString & " where codart = '" & CodigoMercancia & "' and id_emp = '" & jytsistema.WorkID & "' ")

        EjecutarSTRSQL(MyConn, lblInfo, " UPDATE jsmerctainv a, (SELECT a.codart, a.almacen, c.unidad,  " _
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " update jsmerctainv set montoultimacompra = " & UltimoCostoAFecha(MyConn, lblInfo, CodigoMercancia, jytsistema.sFechadeTrabajo) & " where codart = '" & CodigoMercancia & "' and id_emp = '" & jytsistema.WorkID & "' ")
    ...
    Public Sub ActualizaCostosEnCombo(MyConn As MySqlConnection, lblInfo As Label, CodigoMercancia As String)
        ...
        For Each nRow As DataRow In dt.Rows
            EjecutarSTRSQL(MyConn, lblInfo, " update jsmerctainv set montoultimacompra =  " & CDbl(EjecutarSTRSQL_ScalarPLUS(MyConn, " SELECT SUM(b.montoultimacompra) montoultimacompra " _
    ...
    End Sub
    Public Sub ActualizaExistenciasEnAlmacen(ByVal MyConn As MySqlConnection, ByVal CodigoMercancia As String, ByVal lblInfo As Label)

        Dim ds As New DataSet
        Dim dt As DataTable
        Dim nTabla As String = "tblexistealm"
        Dim kCont As Integer

        ds = DataSetRequery(ds, " SELECT a.codart, a.almacen, c.unidad,  " _
                    & " ROUND( SUM(IF( a.tipomov IN ('EN', 'AE', 'DV') ,  a.cantidad, if ( a.tipomov IN ('SA','AS','DC'), -1*a.cantidad, 0 )) *(IF( ISNULL(b.uvalencia), 1, 1/b.equivale  ))), 3) Existencia " _
                    & " FROM jsmertramer a  " _
                    & " LEFT JOIN jsmerequmer b ON (a.codart = b.codart AND a.unidad = b.uvalencia AND a.id_emp = b.id_emp)  " _
                    & " LEFT JOIN jsmerctainv c ON (a.codart = c.codart AND a.id_emp = c.id_emp)  " _
                    ...
                    & " a.codart = '" & CodigoMercancia & "' AND " _
                    & " a.id_emp = '" & jytsistema.WorkID & "' " _
                    & " GROUP BY a.codart, a.almacen  ", MyConn, nTabla, lblInfo)

        dt = ds.Tables(nTabla)

        If dt.Rows.Count > 0 Then
            For kCont = 0 To dt.Rows.Count - 1
                Dim aFld() As String = {"codart", "almacen", "id_emp"}
                With dt.Rows(kCont)
                    Dim aSTR() As String = {.Item("codart"), .Item("almacen"), jytsistema.WorkID}
                    If qFound(MyConn, lblInfo, "jsmerextalm", aFld, aSTR) Then
                        EjecutarSTRSQL(MyConn, lblInfo, " update jsmerextalm set existencia = " & .Item("existencia") & " where codart = '" & .Item("codart") & "' and almacen = '" & .Item("almacen") & "' and id_emp = '" & jytsistema.WorkID & "' ")
                    Else
                        EjecutarSTRSQL(MyConn, lblInfo, " insert into jsmerextalm set codart = '" & .Item("codart") & "', almacen = '" & .Item("almacen") & "', existencia = " & .Item("existencia") & " , ubicacion = '', id_emp = '" & jytsistema.WorkID & "'  ")
                    End If
                End With
            Next
        End If
    ...
    Public Function UltimoCostoAFecha(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoMercancia As String, ByVal Fecha As Date) As Double

        If Mid(CodigoMercancia, 1, 1) <> "$" Then
            UltimoCostoAFecha = CDbl(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " select ROUND(SUM(a.costotaldes)/if( SUM(IF (ISNULL(UVALENCIA), a.CANTIDAD, a.cantidad/b.equivale)) > 0, SUM(IF (ISNULL(UVALENCIA), a.CANTIDAD, a.cantidad/b.equivale)) ,1) ,2) costotal " _
                & " FROM jsmertramer a " _
                & " LEFT JOIN jsmerequmer b ON ( a.codart = b.CODART AND a.unidad = b.UVALENCIA AND a.ID_EMP = b.ID_EMP ) " _
                & " WHERE " _
                & " a.codart = '" & CodigoMercancia & "' AND " _
                & " a.fechamov <= '" & FormatoFechaHoraMySQLFinal(Fecha) & "' AND " _
    ...
    Public Function UltimoCostoMenorAFecha(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoMercancia As String, ByVal Fecha As Date, nCosto As Double) As Double
        ...
        If Mid(CodigoMercancia, 1, 1) <> "$" Then
            ...
            aArr = EjecutarSTRSQL_Reader(MyConn, lblInfo, " select ROUND(SUM(a.costotaldes)/if( SUM(IF (ISNULL(UVALENCIA), a.CANTIDAD, a.cantidad/b.equivale)) > 0, SUM(IF (ISNULL(UVALENCIA), a.CANTIDAD, a.cantidad/b.equivale)) ,1) ,2) costotal " _
                & " FROM jsmertramer a LEFT JOIN jsmerequmer b ON ( a.codart = b.CODART AND a.unidad = b.UVALENCIA AND a.ID_EMP = b.ID_EMP ) WHERE " _
                & " a.codart = '" & CodigoMercancia & "' AND " _
                & " a.fechamov <= '" & FormatoFechaHoraMySQL(Fecha) & "' AND " _
                & " a.tipomov IN ('EN', 'AC', 'AE') AND " _
                & " a.origen IN ('COM', 'INV') AND " _
                & " a.ID_EMP = '" & jytsistema.WorkID & "' " _
                & " GROUP BY a.codart, a.numdoc " _
    ...
    Public Function UltimoProveedor(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoMercancia As String, ByVal Fecha As Date) As String
        ...
        Return EjecutarSTRSQL_Scalar(MyConn, lblInfo, " select prov_cli " _
               & " FROM jsmertramer a " _
               & " WHERE " _
               & " a.codart = '" & CodigoMercancia & "' AND " _
               & " a.fechamov <= '" & FormatoFechaHoraMySQLFinal(Fecha) & "' AND " _
    ...
    Public Function UltimaFechaCompra(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoMercancia As String, ByVal Fecha As Date) As Date
        ...
        Dim ultimafechaDeCompra As String = EjecutarSTRSQL_Scalar(MyConn, lblInfo, " select fechamov " _
               & " FROM jsmertramer a " _
               & " WHERE " _
               & " a.codart = '" & CodigoMercancia & "' AND " _
               & " a.fechamov <= '" & FormatoFechaHoraMySQLFinal(Fecha) & "' AND " _
               ...
               & " a.ID_EMP = '" & jytsistema.WorkID & "' " _
               & " GROUP BY a.codart, a.numdoc " _
               & " ORDER BY a.fechamov DESC, a.origen LIMIT 1").ToString

        If ultimafechaDeCompra = "0" Then
            Return CDate(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " select creacion " _
               & " FROM jsmerctainv a " _
               & " WHERE " _
               & " a.codart = '" & CodigoMercancia & "' AND " _
               & " a.ID_EMP = '" & jytsistema.WorkID & "' " _
    ...
    Public Function Equivalencia(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoMercancia As String, ByVal Unidad As String) As Double
        Dim Equivale As Double = CDbl(EjecutarSTRSQL_Scalar(MyConn, lblInfo, "SELECT equivale FROM jsmerequmer where CODART = '" & CodigoMercancia & "' AND UVALENCIA = '" & Unidad & "' AND ID_EMP = '" & jytsistema.WorkID & "' "))
    ...
    Public Sub EliminarMovimientosdeInventario(ByVal MyConn As MySqlConnection, ByVal Documento As String, ByVal Origen As String, _
        ...
        EjecutarSTRSQL(MyConn, lblinfo, "delete FROM jsmertramer " _
            & " where " _
            & str _
    ...
    Public Sub CargaListViewCombosConExistencia(ByVal LV As ListView, ByVal dt As DataTable)
        ...
        For iCont = 0 To dt.Rows.Count - 1
            ...
            With dt.Rows(iCont)
                ...
                LV.Items(iCont).SubItems.Add(FormatoCantidad(CDbl(.Item("existencia").ToString)))
    ...
    Function CantidadEquivalente(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        lEquivalencia = Equivalencia(MyConn, lblInfo, sArticulo, sUnidad)
    ...
    Function MercanciaPoseeEquivalencia(MyConn As MySqlConnection, lblInfo As Label, CodigoArticulo As String, UnidadAVerificar As String) As Boolean
        ...
        Dim nInt As Integer = CInt(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " SELECT COUNT(*) FROM (SELECT unidad FROM jsmerctainv " _
                                                           & " WHERE " _
                                                           & " codart = '" & CodigoArticulo & "' and " _
    ...
    'Function CostoUnidadVentaAlaFecha(MyConn As MySqlConnection, ds As DataSet, lblInfo As Label, CodigoArticulo As String, Fecha As Date) As Double

    '    Dim dt As DataTable
    '    Dim nTable As String = "tbl"

    '    ds = DataSetRequery(ds, "select a.CODART, a.fechamov, a.numdoc, a.UNIDAD, SUM(a.CANTIDAD) as Cantidadrenglon, " _
    '        & " SUM(IF (ISNULL(b.UVALENCIA), a.CANTIDAD, a.cantidad/b.equivale)) as CANTIDAD, " _
    '        & " SUM(a.costotaldes) AS costotal, a.PROV_CLI " _
    '        & " from jsmertramer a " _
    '        & " left join jsmerequmer b ON  a.codart = b.CODART AND a.UNIDAD = b.UVALENCIA  and a.ID_EMP = a.ID_EMP " _
    '        & " Where " _
    '        & " a.codart = '" & CodigoArticulo & "' AND " _
    '        & " a.fechamov < '" & FormatoFechaHoraMySQLDiaMas(Fecha) & "' and " _
    '        & " (a.tipomov = 'EN' or a.tipomov = 'AC' or a.tipomov = 'AE') and " _
    '        & " (a.origen = 'COM' OR a.origen = 'INV') and " _
    '        & " a.ID_EMP = '" & jytsistema.WorkID & "' and " _
    '        & " a.EJERCICIO = '" & jytsistema.WorkExercise & "' " _
    '        & " GROUP BY a.codart, a.numdoc " _
    '        & " order by a.fechamov desc limit 1 ", MyConn, nTable, lblInfo)

    '    dt = ds.Tables(nTable)
    '    CostoUnidadVentaAlaFecha = 0
    '    If dt.Rows.Count > 0 Then CostoUnidadVentaAlaFecha = dt.Rows(0).Item("COSTOTAL") / IIf(dt.Rows(0).Item("CANTIDAD") > 0, dt.Rows(0).Item("cantidad"), 1)
