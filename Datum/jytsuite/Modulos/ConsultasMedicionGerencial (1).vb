...
Module ConsultasMedicionGerencial
    Function SeleccionSIGMEGananciasBrutasFacturas(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        tblGanancias = "tbl" & Format(NumeroAleatorio(10000), "000000")

        Dim aFld() As String = {"prov_cli.cadena15", "vendedor.cadena5", "NUMDOC.cadena30", "fechamov.fechahora", "codart.cadena15", _
                                "unidad.cadena3", "signo.entero4", "cantidad.doble10", "equivale.doble10", "costotal.doble19", _
                                "costotaldes.doble19", "ventotal.doble19", "ventotaldes.doble19", "id_emp.cadena2"}
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tblGanancias & " select a.prov_cli, a.vendedor, a.numdoc, a.fechamov, a.codart, a.unidad, if( tipomov in ('EN','AE'), -1, 1) signo,  sum(a.cantidad) as cantidad, " _
    ...
    Function SeleccionSIGMEGananciasBrutasItem(ByVal myConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        tblGanancias = "tbl" & Format(NumeroAleatorio(10000), "000000")
        Dim aFld() As String = {"prov_cli.cadena15", "vendedor.cadena5", "codart.cadena15", _
                               "unidad.cadena3", "signo.entero4", "cantidad.doble10", "equivale.doble10", "costotal.doble19", _
                               "costotaldes.doble19", "ventotal.doble19", "ventotaldes.doble19", "id_emp.cadena2"}
        ...
        EjecutarSTRSQL(myConn, lblInfo, " insert into " & tblGanancias & " select a.prov_cli, a.vendedor, a.codart, a.unidad, if( tipomov in ('SA', 'AS') , 1,-1) as signo,  sum(a.cantidad) as cantidad, " _
            & " if( isnull(b.equivale),1, b.equivale) as equivale,  sum(a.costotal) as costotal, " _
            & " sum(a.costotaldes) as costotaldes, sum(a.ventotal) as ventotal, sum(ventotaldes) as ventotaldes, a.id_emp " _
            & " from jsmertramer a " _
            & " left join jsmerequmer b on (a.codart = b.codart and a.unidad = b.uvalencia and a.id_emp = b.id_emp) " _
            & " Where " _
            & strOrigen _
            & " a.fechamov >= '" & FormatoFechaHoraMySQLInicial(FechaDesde) & "' AND " _
            & " a.FECHAMOV <= '" & FormatoFechaHoraMySQLFinal(FechaHasta) & "' and " _
            & " a.id_emp  = '" & jytsistema.WorkID & "'  " _
            & " group By " _
            & " a.prov_cli, a.vendedor, a.codart, a.unidad, signo ")
        ...
        'EjecutarSTRSQL(myConn, lblInfo, " drop table " & tblGanancias)
    ...
    Function SeleccionSIGMEGananciasBrutasMesMes(ByVal myConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        SeleccionSIGMEGananciasBrutasMesMes = "SELECT a.numdoc, a.codart, c.nomart, c.unidad, a.fechamov, " & st01 & st02 & st03 & st04 & st05 & st06 & st07 & st08 & st09 & st10 & st11 & st12 _
            ...
            & " a.fechamov >= '" & FormatoFechaHoraMySQLInicial(PrimerDiaAño(FechaDesde)) & "' AND " _
            & " a.fechamov <= '" & FormatoFechaHoraMySQLFinal(UltimoDiaAño(FechaHasta)) & "' " _
    ...
    Optional ByVal DivisionDesde As String = "", Optional ByVal DivisionHasta As String = "") As String
        ...
        tblGanancias = "tbl" & Format(NumeroAleatorio(10000), "000000")
        EjecutarSTRSQL(myConn, lblInfo, "create temporary table " & tblGanancias & "( " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " insert into " & tblGanancias & " select a.prov_cli, a.vendedor, a.numdoc, a.fechamov, a.codart, a.unidad, if(tipomov in ('EN', 'AE'), -1, 1) as signo,  sum(a.cantidad) as cantidad, " _
            & " if( isnull(b.equivale),1, b.equivale) as equivale,  sum(a.costotal) as costotal, " _
    ...
    Function SeleccionSIGMEIngresosporAsesoryForma(ByVal Myconn As MySqlConnection, ByVal lblInfo As Label, ByVal AsesorDesde As String, ByVal AsesorHasta As String, _
        ...
        SeleccionSIGMEIngresosporAsesoryForma = " select sum(a.importe)*(-1) monto, concat(a.formapag,'     ') as forma,if (a.formapag='EF','EFECTIVO                  ',if (a.formapag='CH','CHEQUE     ','DEPOSITO    ')) as formadescrip, " _
            ...
            & " and a.emision <= '" & FormatoFechaMySQL(FechaHasta) & "' and a.formapag in ('CH','DP','EF') " & strSQLAsesor1 _
            & " group by 2,6 " _
            & " Union " _
            & " select sum(a.importe) as monto,CONCAT(a.formpag,a.refpag) as forma , concat('CHEQUE ALIMENTACION ', a.refpag) as formadescrip , " _
            ...
            & " and a.formpag='CT' " _
            & " and a.fecha >= '" & FormatoFechaMySQL(FechaDesde) & "' " _
            & " and a.fecha <= '" & FormatoFechaMySQL(FechaHasta) & "' " & strSQLAsesor2 _
            & " group by 2,4,6 " _
    ...
    Function SeleccionSIGMEVentasyComprasMesaMes(ByVal myConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        'EjecutarSTRSQL(myConn, lblInfo, " create temporary table " & tblVentas_jer & " (jerarquia  char(2),mes1 double (19,3), mes2 double (19,3),mes3 double (19,3),mes4 double (19,3),mes5 double (19,3),  mes6 double (19,3),mes7 double (19,3),mes8 double (19,3),mes9 double (19,3),mes10 double (19,3),  mes11 double (19,3),mes12 double (19,3))")
        ...
        '        EjecutarSTRSQL(myConn, lblInfo, " create temporary table " & tblcompras_jer & " (jerarquia  char(2),mes1 double (19,3), mes2 double (19,3),mes3 double (19,3),mes4 double (19,3),mes5 double (19,3), mes6 double (19,3),mes7 double (19,3),mes8 double (19,3),mes9 double (19,3),mes10 double (19,3), mes11 double (19,3),mes12 double (19,3)) ")
    ...
    Function SeleccionSIGMEDescuentos(ByVal myConn As MySqlConnection, ByVal lblInfo As Label, ByVal FechaDesde As Date, ByVal FechaHasta As Date, _
        ...
        'tblDescuentos = "tbl" & Format(NumeroAleatorio(10000), "00000")
        

        'EjecutarSTRSQL(myConn, lblInfo, " create temporary table " & tblDescuentos & " (tipo integer(1) default '0' not null, codcli varchar(15) not null, numdoc varchar(15) not null, codart varchar(15) not null, unidad char(3) not null, " _
        ...
        'EjecutarSTRSQL(myConn, lblInfo, " create index " & tblDescuentos & " on " & tblDescuentos & " (tipo, codcli, codart ) ")

        ' Descuentos Generales
        'EjecutarSTRSQL(myConn, lblInfo, " insert into " & tblDescuentos _
        ...
        & " Where " _
        & strSQLCodven _
        & " b.estatus = '2' AND a.emision >= '" & FormatoFechaMySQL(FechaDesde) & "' and a.emision <= '" & FormatoFechaMySQL(FechaHasta) & "' and " _
        ...
        strDescuentos += " union  (select 4 tipo, a.codcli, a.numfac numdoc, b.item codart, d.unidad, " _
            ...
            & " Where " _
            & strSQLCodven _
            & " b.des_ofe <> 0.00 and b.estatus <> '2' and a.emision >= '" & FormatoFechaMySQL(FechaDesde) & "' and a.emision <= '" & FormatoFechaMySQL(FechaHasta) & "' and " _
            & " a.id_emp = '" & jytsistema.WorkID & "' group by 2,3,4 ) "
        ...
        strDescuentos += " union (select  5 tipo, a.codcli , a.numncr numdoc, b.item codart , d.unidad, " _
                ...
                & " a.emision >= '" & FormatoFechaMySQL(FechaDesde) & "' and  a.emision <= '" & FormatoFechaMySQL(FechaHasta) & "' and " _
                & " a.id_emp='" & jytsistema.WorkID & "' " _
                & " Group By 2,3,4 ) "
    ...
    Function SeleccionSIGMEVentasMesMes(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label,
                            ...
                            ByVal AgrupadoPor As String) As String
        ...
        SeleccionSIGMEVentasMesMes = " select a.vendedor,  concat(c.apellidos,', ',c.nombres) as nombre_vendedor, b.tipjer, d.descrip,  " _
                & " sum(if(month(a.fechamov)='01',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs1, sum(if(month(a.fechamov)='02',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs2, " _
                & " sum(if(month(a.fechamov)='03',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs3, sum(if(month(a.fechamov)='04',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs4, " _
                & " sum(if(month(a.fechamov)='05',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs5, sum(if(month(a.fechamov)='06',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs6, " _
                & " sum(if(month(a.fechamov)='07',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs7, sum(if(month(a.fechamov)='08',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs8, " _
                & " sum(if(month(a.fechamov)='09',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs9, sum(if(month(a.fechamov)='10',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs10, " _
                & " sum(if(month(a.fechamov)='11',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs11, sum(if(month(a.fechamov)='12',if(a.origen in ('FAC','PFC','PVE','NDV'),ventotaldes,ventotaldes*(-1)),0)) bs12 " _
                & " from jsmertramer a " _
                & " left join jsmerctainv b on (a.codart = b.codart and a.id_emp = b.id_emp) " _
                ...
                & " where a.fechamov >= '" & FormatoFechaHoraMySQLInicial(PrimerDiaAño(CDate("01/01/" & Año.ToString))) & "' " _
                & " and a.fechamov <= '" & FormatoFechaHoraMySQLFinal(UltimoDiaAño(CDate("01/01/" & Año.ToString))) & "' " _
    ...
    Function SeleccionSIGMEGananciasCestaTicketMesMes(ByVal myConn As MySqlConnection, ByVal lblInfo As Label, ByVal Año As Integer) As String
        ...
        EjecutarSTRSQL(myConn, lblInfo, " create temporary table sobres (corredor char(5), fechamov date,numsobre varchar(15))")
        EjecutarSTRSQL(myConn, lblInfo, " insert into sobres " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " create temporary table iva (corredor char(5), mes1 double(16,2),mes2 double(16,2), " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " insert into iva " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " create temporary table comisiones (corredor char(5), mes1 integer, " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " insert into comisiones " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " create temporary table cobrado_clientes (corredor char(5),mes1 double(16,2), " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " insert into cobrado_clientes " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " create temporary table entradas_banco (corredor char(5),mes1 double(16,2), " _
        & " mes2 double(16,2),mes3 double(16,2),mes4 double(16,2),mes5 double(16,2),mes6 double(16,2), " _
        & " mes7 double(16,2),mes8 double(16,2),mes9 double(16,2),mes10 double(16,2),mes11 double(16,2),mes12 double(16,2)) ")

        EjecutarSTRSQL(myConn, lblInfo, " insert into entradas_banco select b.corredor,sum(if(month(b.fechamov)='01',a.importe,0)),sum(if(month(b.fechamov)='02',a.importe,0)), " _
    ...
    Function SeleccionSIGMEGananciaCestaTicketEnPeriodo(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        tblsobres = "tbl" & NumeroAleatorio(10000)
        tblIVA = "tbl" & NumeroAleatorio(10000)
        tblComisiones = "tbl" & NumeroAleatorio(10000)
        tblentradas_banco = "tbl" & NumeroAleatorio(10000)

        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tblsobres & " (corredor char(5), fechamov date, numsobre varchar(15), montoideal double(19,2) default '0.00' not null, comisionideal double(19,2) default '0.00' not null) ")
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tblsobres & " select b.corredor, a.fechamov, a.numdoc , sum(b.monto), sum(b.comision) " _
            & " from jsbantraban a, jsventabtic b " _
            & " Where " _
            & " a.fechamov >= '" & FormatoFechaMySQL(FechaDesde) & "' and " _
            & " a.fechamov <= '" & FormatoFechaMySQL(FechaHasta) & "' and " _
            & " a.tiporg = 'CT' and " _
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tblIVA & " (corredor char(5), iva double(19,2)) ")
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tblComisiones & " (corredor char(5), comisiones integer) ")
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tblentradas_banco & "(corredor char(5), entrada double(19,2) ) ")
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tblentradas_banco _
    ...
    Function SeleccionSIGMEActivacion(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        Else
            strsqlEmision = " and b.fechamov >= '" & FormatoFechaHoraMySQLInicial(EmisionDesde) & "' "
            strsqlEmision += " and b.fechamov <= '" & FormatoFechaHoraMySQLFinal(EmisionHasta) & "' "
        End If
    ...
    Function SeleccionSIGMEVentasComparativasMesMes(ByVal MyConn As MySqlConnection, ByVal lbInfo As Label, _
        ...
        tblComparativa = "tbl" & Format(NumeroAleatorio(10000), "00000")
        ...
        If AsesorDesde <> "" Then strClientes += " a.codven >= '" & AsesorDesde & "' and "
        If AsesorHasta <> "" Then strClientes += " a.codven <= '" & AsesorHasta & "' and "

        SeleccionSIGMEVentasComparativasMesMes = " select a.codart, c.nomart, c.unidad, " _
            & " c.grupo categoria, c.marca, elt(mix+1,'ECONOMICO','ESTANDAR','SUPERIOR') mix, c.division, c.tipjer, c.codjer, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 1, " & strCan & ", 0.00 )) eneAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 1, " & strCan & ", 0.00 )) eneActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 2, " & strCan & ", 0.00 )) febAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 2, " & strCan & ", 0.00 )) febActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 3, " & strCan & ", 0.00 )) marAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 3, " & strCan & ", 0.00 )) marActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 4, " & strCan & ", 0.00 )) abrAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 4, " & strCan & ", 0.00 )) abrActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 5, " & strCan & ", 0.00 )) mayAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 5, " & strCan & ", 0.00 )) mayActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 6, " & strCan & ", 0.00 )) junAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 6, " & strCan & ", 0.00 )) junActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 7, " & strCan & ", 0.00 )) julAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 7, " & strCan & ", 0.00 )) julActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 8, " & strCan & ", 0.00 )) agoAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 8, " & strCan & ", 0.00 )) agoActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 9, " & strCan & ", 0.00 )) sepAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 9, " & strCan & ", 0.00 )) sepActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 10, " & strCan & ", 0.00 )) octAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 10, " & strCan & ", 0.00 )) octActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 11, " & strCan & ", 0.00 )) novAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 11, " & strCan & ", 0.00 )) novActual, " _
            & " sum(if(year(a.fechamov) = " & AñoAnterior & " and month(a.fechamov) = 12, " & strCan & ", 0.00 )) dicAnterior, sum(if(year(a.fechamov) = " & AñoActual & " and month(a.fechamov) = 12, " & strCan & ", 0.00 )) dicActual " _
            & " from jsmertramer a " _
            ...
            & " a.origen in ('FAC','NDV','PFC','PVE') and " _
            & " a.id_emp = '" & jytsistema.WorkID & "' " _
            & " group by a.codart "
    ...
    Function SeleccionSIGMEDropSize(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        tbldropsize = "tbl" & Format(NumeroAleatorio(10000), "00000")
        tbldropsize_b = "tbl" & Format(NumeroAleatorio(10000), "00000")
        tbldropsize_c = "tbl" & Format(NumeroAleatorio(10000), "00000")
        tblcantidad = "tbl" & Format(NumeroAleatorio(10000), "00000")
        tblkilos = "tbl" & Format(NumeroAleatorio(10000), "00000")
        tblMoney = "tbl" & Format(NumeroAleatorio(10000), "00000")
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary TABLE " & tbldropsize & " (tipo int(1) default '0' not null,  codigo varchar(55) not null, mEne Double(19,2) default '0.00' not null, mFeb Double(19,2) default '0.00' not null, mMar Double(19,2) default '0.00' not null, mAbr Double(19,2) default '0.00' not null, mMay Double(19,2) default '0.00' not null, mJun Double(19,2) default '0.00' not null, mJul Double(19,2) default '0.00' not null, mAgo Double(19,2) default '0.00' not null, mSep Double(19,2) default '0.00' not null, mOct Double(19,2) default '0.00' not null, mNov Double(19,2) default '0.00' not null, mDic Double(19,2) default '0.00' not null)")
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary TABLE " & tbldropsize_b & " (tipo int(1) default '0' not null, codigo varchar(55) not null, mEne Double(19,2) default '0.00' not null, mFeb Double(19,2) default '0.00' not null, mMar Double(19,2) default '0.00' not null, mAbr Double(19,2) default '0.00' not null, mMay Double(19,2) default '0.00' not null, mJun Double(19,2) default '0.00' not null, mJul Double(19,2) default '0.00' not null, mAgo Double(19,2) default '0.00' not null, mSep Double(19,2) default '0.00' not null, mOct Double(19,2) default '0.00' not null, mNov Double(19,2) default '0.00' not null, mDic Double(19,2) default '0.00' not null)")
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary TABLE " & tbldropsize_c & " (tipo int(1) default '0' not null, codigo varchar(55) not null, mEne Double(19,2) default '0.00' not null, mFeb Double(19,2) default '0.00' not null, mMar Double(19,2) default '0.00' not null, mAbr Double(19,2) default '0.00' not null, mMay Double(19,2) default '0.00' not null, mJun Double(19,2) default '0.00' not null, mJul Double(19,2) default '0.00' not null, mAgo Double(19,2) default '0.00' not null, mSep Double(19,2) default '0.00' not null, mOct Double(19,2) default '0.00' not null, mNov Double(19,2) default '0.00' not null, mDic Double(19,2) default '0.00' not null)")
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary TABLE " & tblcantidad & " (codart varchar(15) not null, mEne Double(19,2) default '0.00' not null, mFeb Double(19,2) default '0.00' not null, mMar Double(19,2) default '0.00' not null, mAbr Double(19,2) default '0.00' not null, mMay Double(19,2) default '0.00' not null, mJun Double(19,2) default '0.00' not null, mJul Double(19,2) default '0.00' not null, mAgo Double(19,2) default '0.00' not null, mSep Double(19,2) default '0.00' not null, mOct Double(19,2) default '0.00' not null, mNov Double(19,2) default '0.00' not null, mDic Double(19,2) default '0.00' not null) ")
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary TABLE " & tblkilos & " (codart varchar(15) not null, mEne Double(19,2) default '0.00' not null, mFeb Double(19,2) default '0.00' not null, mMar Double(19,2) default '0.00' not null, mAbr Double(19,2) default '0.00' not null, mMay Double(19,2) default '0.00' not null, mJun Double(19,2) default '0.00' not null, mJul Double(19,2) default '0.00' not null, mAgo Double(19,2) default '0.00' not null, mSep Double(19,2) default '0.00' not null, mOct Double(19,2) default '0.00' not null, mNov Double(19,2) default '0.00' not null, mDic Double(19,2) default '0.00' not null) ")
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary TABLE " & tblMoney & " (codart varchar(15) not null, mEne Double(19,2) default '0.00' not null, mFeb Double(19,2) default '0.00' not null, mMar Double(19,2) default '0.00' not null, mAbr Double(19,2) default '0.00' not null, mMay Double(19,2) default '0.00' not null, mJun Double(19,2) default '0.00' not null, mJul Double(19,2) default '0.00' not null, mAgo Double(19,2) default '0.00' not null, mSep Double(19,2) default '0.00' not null, mOct Double(19,2) default '0.00' not null, mNov Double(19,2) default '0.00' not null, mDic Double(19,2) default '0.00' not null) ")

        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tblcantidad _
            & " select a.item, " _
            ...
            & " a.id_emp = '" & jytsistema.WorkID & "' " & strMercas & strClientes _
            & " GROUP by a.item")

        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tblkilos _
            & " Select a.item, " _
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tblMoney _
        & " Select a.item, " _
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tbldropsize _
        ...
        & " a.id_emp = '" & jytsistema.WorkID & "' " & strClientes & " ")
        ...
        & " Where " _
        & " year(a.emision) = " & Año & " and " _
        & " a.id_emp = '" & jytsistema.WorkID & "'" & strMercas & strClientes)

        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tbldropsize _
            & " select 0, 'Visitas', " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/01/" & CStr(Año)))) & "', d.codcli, null)) mEne, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/02/" & CStr(Año)))) & "', d.codcli, null)) mfeb, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/03/" & CStr(Año)))) & "', d.codcli, null)) mMar, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/04/" & CStr(Año)))) & "', d.codcli, null)) mAbr, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/05/" & CStr(Año)))) & "', d.codcli, null)) mMay, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/06/" & CStr(Año)))) & "', d.codcli, null)) mJun, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/07/" & CStr(Año)))) & "', d.codcli, null)) mJul, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/08/" & CStr(Año)))) & "', d.codcli, null)) mAgo, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/09/" & CStr(Año)))) & "', d.codcli, null)) mSep, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/10/" & CStr(Año)))) & "', d.codcli, null)) mOct, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/11/" & CStr(Año)))) & "', d.codcli, null)) mNov, " _
            & " count(IF( d.INGRESO <= '" & FormatoFechaMySQL(UltimoDiaMes(CDate("01/12/" & CStr(Año)))) & "', d.codcli, null)) mDic " _
            & " from jsvencatcli d " _
            & " Where " _
            & " d.ruta_visita <> '' and " _
            & " d.estatus <= 1 and " _
            & " d.id_emp = '" & jytsistema.WorkID & "'" & strClientes)
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tbldropsize_b & " select * FROM " & tbldropsize)
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tbldropsize_c & " select * FROM " & tbldropsize)
