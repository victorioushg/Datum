...
Module FuncionesVentas
        ...
        If dtFac.Rows.Count > 0 Then
            For Each nRow As DataRow In dtFac.Rows
                With nRow
                    ...
                    If CBool(EjecutarSTRSQL_ScalarPLUS(MyConn, "SELECT CUOTAFIJA FROM jsmerctainv WHERE CODART = '" & .Item("ITEM") & "' AND ID_EMP = '" & jytsistema.WorkID & "' ")) Then
    ...
    Public Function DiasCreditoAlVencimiento(ByVal MyConn As MySqlConnection, ByVal FormaDePago As String) As Integer
        DiasCreditoAlVencimiento = CInt(EjecutarSTRSQL_ScalarPLUS(MyConn, " select periodo from jsconctafor where codfor = '" & FormaDePago & "' and id_emp = '" & jytsistema.WorkID & "' "))
    End Function

    Public Function FechaVencimientoFactura(MyConn As MySqlConnection, CodigoCliente As String, FechaEmision As Date) As Date
        Dim FormaDePagoCliente As String = EjecutarSTRSQL_ScalarPLUS(MyConn, " select formapago from jsvencatcli where codcli = '" & CodigoCliente & "' and id_emp = '" & jytsistema.WorkID & "' ")
    ...
    Public Function AsesoresDeSupervisor(MyConn As MySqlConnection, lblInfo As Label, CodigoSupervisor As String, Optional TipoSupervisor As Integer = 1) As String
        ...
        Select Case TipoSupervisor
            ...
            Case Else
                nVendedores = EjecutarSTRSQL_Reader(MyConn, lblInfo, "SELECT codven FROM jsvencatven WHERE tipo = 0 AND id_emp = '" & jytsistema.WorkID & "' ")
        End Select
    ...
    Function CuotaAsesorMES_KGR(MyConn As MySqlConnection, CodigoAsesor As String, CodigoMercancia As String, Fecha As Date) As Double
        Return CDbl(EjecutarSTRSQL_ScalarPLUS(MyConn, " SELECT  ESMES" + Format(Fecha.Month, "00") + " " _
                                           & " FROM jsvencuoart " _
                                           & " WHERE " _
