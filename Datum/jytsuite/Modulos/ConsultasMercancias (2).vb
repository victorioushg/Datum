...
Module ConsultasMercancias
    ...
    Function SeleccionMERMercancias(ByVal MercanciaDesde As String, ByVal MercanciaHasta As String, ByVal OrdenadoPor As String, ByVal Operador As Integer, _
        ...
        SeleccionMERMercancias = "select a.codart, a.nomart, a.alterno, a.barras, date_format(a.creacion,'%d-%m-%Y') creacion, " _
              ...
              & " left join (select fecha, tipo, monto from jsconctaiva where fecha in (select MAX(fecha) from jsconctaiva where fecha <= '" & FormatoFechaMySQL(jytsistema.sFechadeTrabajo) & "' group by tipo)) b on (a.iva = b.tipo) " _
    ...
    Function SeleccionMERCodigoBarras(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal CodigoMercancia As String) As String
        Dim cod As String = Codigo128(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " select a.barras from jsmerctainv a where a.codart = '" & CodigoMercancia & "' and a.id_emp = '" & jytsistema.WorkID & "'  ")).Replace("'", String.Empty)
        ...
        Return "  SELECT a.codart, '" & cod & "' barras, a.barras barrasreal, a.nomart, " _
            ...
            & " ROUND( (IF( ISNULL(b.monto), 0, b.monto )/100 + 1) *a.precio_F/IF( ISNULL(c.equivale), 1, c.equivale), 2) PRECIO_F " _
            & " FROM jsmerctainv a " _
            & " LEFT JOIN (SELECT fecha, tipo, monto FROM jsconctaiva WHERE fecha IN (SELECT MAX(fecha) FROM jsconctaiva WHERE fecha <= '" & _
            FormatoFechaMySQL(jytsistema.sFechadeTrabajo) & "' GROUP BY tipo)) b ON (a.iva = b.tipo) " _
    ...
    End Function
    Function SeleccionMERInventarioLegal(ByVal MercanciaDesde As String, ByVal MercanciaHasta As String, ByVal OrdenadoPor As String, ByVal Operador As Integer, _
        ...
        Dim strFechaInicial As String = FormatoFechaMySQL(DateAdd(DateInterval.Day, -1, FechaInicial))
        Dim strFechaFinal As String = FormatoFechaMySQL(DateAdd(DateInterval.Day, 1, FechaFinal))

        SeleccionMERInventarioLegal = " SELECT a.codart, a.nomart, a.grupo, a.marca, a.division, a.unidad, IF( b.existencia IS NULL, 0.00, IF(b.existencia = 0.00, 0.00, b.existencia)) InventarioInicial,  " _
                & " IF (b.entradas IS NULL, 0.00, b.entradas) Entradas, " _
                & " IF (b.salidas IS NULL, 0.00, b.salidas) salidas, " _
                & " IF (b.costoi IS NULL, 0.00,  b.costoi) CostoInicial, IF (b.costoe IS NULL, 0.00,  b.costoe) CostoEntradas, " _
                & " IF (b.costof IS NULL, 0.00,  b.costof) Costopromedio,  if( b.costofinaltotal is null, 0.00, b.costofinaltotal ) costofinaltotal,  " _
                ...
                & "             SUM(a.costoini)/SUM( IF( ISNULL(f.uvalencia), a.entradastotales_i, a.entradastotales_i/f.equivale ) ) costoi, " _
                & "             SUM(a.costo)/SUM( IF( ISNULL(f.uvalencia), a.entradastotales, a.entradastotales/f.equivale ) ) costof , " _
                & "             SUM(a.costo) costofinaltotal , " _
                & "             SUM(a.costoentradas)/SUM( IF( ISNULL(f.uvalencia), a.entradas, a.entradas/f.equivale ) ) costoe, " _
                ...
                & "             FROM (SELECT b.codart, b.unidad, SUM( IF( b.fechamov < '" & FormatoFechaMySQL(FechaInicial) & "',  IF( b.TIPOMOV IN ('EN','AE','DV') , b.cantidad, if( b.tipomov in('SA','AS','DC'), -1 * b.cantidad, 0)), 0 ) ) existencia, " _
                & "                     SUM( IF ( b.tipomov IN ('EN','AE','DV','AC'), b.costotaldes, 0.00)) costo, " _
                & "                     SUM( IF ( b.tipomov IN ('EN','AE','DV','AC') and b.fechamov < '" & FormatoFechaMySQL(FechaInicial) & "', b.costotaldes, 0.00)) costoini, " _
                & "                     SUM( if ( b.tipomov IN ('EN','AE','DV') and b.fechamov < '" & FormatoFechaMySQL(FechaInicial) & "', b.cantidad, 0.00)) entradastotales_i, " _
                & "                     SUM( IF ( b.tipomov IN ('EN','AE','DV','AC') and b.fechamov > '" & strFechaInicial & "', b.costotaldes, 0.00 ) ) costoentradas, " _
                & "                     SUM( if ( b.tipomov IN ('EN','AE','DV'), b.cantidad, 0.00)) entradastotales, " _
                & "                     SUM( IF ( b.fechamov > '" & strFechaInicial & "',   IF( b.tipomov IN ('EN','AE','DV'), b.cantidad, 0.00), 0.00)) entradas, " _
                & "                     SUM( IF ( b.fechamov > '" & strFechaInicial & "',   IF( b.tipomov IN ('SA','AS','DC'), b.cantidad, 0.00), 0.00)) salidas FROM jsmertramer b " _
                & "                     WHERE " _
                & "                     b.id_emp = '" & jytsistema.WorkID & "' AND " _
                & "                     b.ejercicio = '" & jytsistema.WorkExercise & "' AND " _
                & "                     b.fechamov < '" & strFechaFinal & "' " _
                & strAlm & "        GROUP BY b.codart, b.unidad ) a " _
    ...
    Function SeleccionMERInventarioLegalPlus(ByVal MercanciaDesde As String, ByVal MercanciaHasta As String, ByVal OrdenadoPor As String, ByVal Operador As Integer, _
        ...
        Dim strFechaInicial As String = FormatoFechaHoraMySQLInicial(FechaInicial)
        Dim strFechaFinal As String = FormatoFechaHoraMySQLFinal(FechaFinal)
    ...
    Function SeleccionMERMercanciasPreciosYEquivalencias(ByVal MercanciaDesde As String, ByVal MercanciaHasta As String, ByVal OrdenadoPor As String, ByVal Operador As Integer, _
        ...
        SeleccionMERMercanciasPreciosYEquivalencias = "select a.codart, a.nomart, a.alterno, a.barras, date_format(a.creacion,'%d-%m-%Y') creacion, " _
              ...
              & " q.uvalencia, ROUND(a.precio_" & aTarifa(Tarifa) & "/q.equivale*(1+IF(b.monto IS NULL, 0.00, b.monto)/100),2) precioEquivalente, ELT(q.divide+1,'No','Si') divideuv " _
              & " from jsmerctainv a " _
              & " left join (select fecha, tipo, monto from jsconctaiva where fecha in (select MAX(fecha) from jsconctaiva where fecha <= '" & FormatoFechaMySQL(jytsistema.sFechadeTrabajo) & "' group by tipo)) b on (a.iva = b.tipo) " _
              & " left join (select a.codart, sum(a.existencia) existencia, a.id_emp from jsmerextalm a where " & strAlm & " a.id_emp = '" & jytsistema.WorkID & "' group by a.codart ) c on (a.codart = c.codart and a.id_emp = c.id_emp) " _
              & " left join jsconctatab j on (a.grupo = j.codigo and a.id_emp = j.id_emp and j.modulo = '00002') " _
    ...
    Function SeleccionMERObsolescencias(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, ByVal OrdenadoPor As String, ByVal Operador As Integer, _
                                    ...
                                    Optional ByVal AsesorDesde As String = "", Optional ByVal AsesorHasta As String = "") As String
        ...
        Dim tbl As String = "tbl" & NumeroAleatorio(100000)
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " UPDATE jsmerctainv a, (SELECT a.codart, DATE_FORMAT(MAX(a.fechamov), '%Y-%m-%d') fecultventa, a.id_emp FROM jsmertramer a " _
                            & "                             WHERE " _
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " CREATE TEMPORARY TABLE " & tbl _
                            & " SELECT a.codart, a.nomart, a.unidad, a.estatus, " _
                            ...
                            & "             FROM jsconcatper WHERE id_emp = '" & jytsistema.WorkID & "') b ON (a.id_emp = b.id_emp) " _
                            & " WHERE " _
                            & " a.id_emp = '" & jytsistema.WorkID & "' " _
                            & " GROUP BY a.codart ")

        EjecutarSTRSQL(MyConn, lblInfo, " UPDATE " & tbl & " a, (SELECT a.codart, ROUND( SUM( IF( a.fechamov > DATE_SUB(c.fecultventa, INTERVAL 3 MONTH), " _
    ...
    Function SeleccionMERMovimientosMercancias(ByVal MercanciaDesde As String, ByVal MercanciaHasta As String, _
        ...
        str += " b.fechamov >= '" & FormatoFechaMySQL(FechaDesde) & "' and "
        str += " b.fechamov <= '" & FormatoFechaMySQL(FechaHasta) & "' and "
    ...
    Function SeleccionMERExistenciasAFecha(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        Dim tbltramer As String = "tbl" & Format(NumeroAleatorio(10000), "000000")
        Dim tblCostos As String = "tbl" & Format(NumeroAleatorio(10000), "000000")
        Dim tblext As String = "tbl" & Format(NumeroAleatorio(10000), "000000")
        Dim tblextReal As String = "tbl" & Format(NumeroAleatorio(10000), "000000")
        Dim tblDiaria As String = "tbl" & Format(NumeroAleatorio(10000), "000000")
        Dim tblDiariaReal = "tbl" & Format(NumeroAleatorio(10000), "000000")

        'existencias
        EjecutarSTRSQL(MyConn, lblInfo, " drop temporary table if exists " & tblext)
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tblext _
            ...
            & " b.id_emp = '" & jytsistema.WorkID & "' and " _
            & " b.ejercicio = '" & jytsistema.WorkExercise & "' and " _
            & " b.fechamov < '" & FormatoFechaMySQL(DateAdd("d", 1, FechaHasta)) & "' " _
            & " group by b.codart, b.unidad ")

        EjecutarSTRSQL(MyConn, lblInfo, " drop temporary table if exists " & tblextReal)
        EjecutarSTRSQL(MyConn, lblInfo, " Create temporary table " & tblextReal _
                        & " select a.codart, sum(if( isnull(f.uvalencia), a.existencia, a.existencia/f.equivale)) existencia " _
                        & " from " & tblext & " a " _
                        & " left join jsmerequmer f on (a.codart = f.codart and a.unidad = f.uvalencia and f.id_emp = '" & jytsistema.WorkID & "') " _
                        & " group by a.codart ")

        Dim DiasNH As Integer = CInt(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " SELECT COUNT(id_emp) FROM jsconcatper WHERE " _
                                                      & " DATE_FORMAT(CONCAT(ano,'-',mes,'-',dia),'%Y-%m-%d') >= '" & FormatoFechaMySQL(DateAdd("d", -1 * diasHabiles, FechaHasta)) & "' AND " _
                                                      & " DATE_FORMAT(CONCAT(ano,'-',mes,'-',dia),'%Y-%m-%d') <= '" & FormatoFechaMySQL(FechaHasta) & "' AND " _
                                                      & " id_emp = '" & jytsistema.WorkID & "' GROUP BY id_emp"))

        EjecutarSTRSQL(MyConn, lblInfo, "Drop temporary table if exists " & tblDiaria)
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tblDiaria _
            & " select b.codart, b.unidad, sum(IF( b.TIPOMOV = 'EN' OR b.tipomov = 'AE' OR b.TIPOMOV = 'DV', 0.00, b.CANTIDAD))/ " _
            & " (to_days('" & FormatoFechaMySQL(FechaHasta) & "') - to_days('" & FormatoFechaMySQL(DateAdd("d", -1 * IIf(diasHabiles = 0, 1, diasHabiles), FechaHasta)) & "') - " & DiasNH & " )  salidas " _
            & " from jsmertramer b " _
            & " Where " _
            & " b.id_emp = '" & jytsistema.WorkID & "' and " _
            & " b.ejercicio = '' and " _
            & " b.fechamov >= '" & FormatoFechaMySQL(DateAdd("d", -1 * diasHabiles, FechaHasta)) & "' and " _
            & " b.fechamov <= '" & FormatoFechaMySQL(FechaHasta) & "' " _
            & " group by b.codart, b.unidad ")

        EjecutarSTRSQL(MyConn, lblInfo, "drop temporary table if exists " & tblDiariaReal)
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tblDiariaReal & " select a.codart, sum(if( isnull(f.uvalencia), a.salidas, a.salidas/f.equivale)) salidasdiarias " _
            & " from " & tblDiaria & " a left join jsmerequmer f on (a.codart = f.codart and a.unidad = f.uvalencia and f.id_emp = '" & jytsistema.WorkID & "') " _
            & " group by a.codart ")

        'costos
        EjecutarSTRSQL(MyConn, lblInfo, " drop temporary table if exists " & tbltramer)
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tbltramer & " (codart varchar(15)  not null, fechamov datetime not null) ")
        EjecutarSTRSQL(MyConn, lblInfo, " create index " & tbltramer & " on " & tbltramer & " (codart, fechamov) ")
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tbltramer & " select codart, max(fechamov) as fechamov from jsmertramer where " _
            & " fechamov <= '" & FormatoFechaMySQL(DateAdd("d", 1, FechaHasta)) & "' and " _
            & " tipomov in ('EN','AE') and " _
            & " origen in ('COM','INV') and " _
            & " id_emp = '" & jytsistema.WorkID & "' " _
            & " group by codart ")

        EjecutarSTRSQL(MyConn, lblInfo, " drop temporary table if exists " & tblCostos)
        EjecutarSTRSQL(MyConn, lblInfo, " create temporary table " & tblCostos & " (codart varchar(15) not null, " _
        ...
        EjecutarSTRSQL(MyConn, lblInfo, " create index " & tblCostos & " on " & tblCostos & " (codart)")
        EjecutarSTRSQL(MyConn, lblInfo, " insert into " & tblCostos & " select a.codart, a.fechamov, a.tipomov, a.numdoc, a.unidad, sum(a.cantidad) cantidad, sum(a.peso) peso, " _
            ...
            & " a.fechamov <= '" & FormatoFechaMySQL(DateAdd("d", 1, FechaHasta)) & "' and " _
        ...
        SeleccionMERExistenciasAFecha = " select elt(a.mix+1,'ECONOMICO','ESTANDAR','SUPERIOR') AS mix, " _
            ...
            & "                     fechamov <= '" & FormatoFechaHoraMySQLFinal(FechaHasta) & "' AND " _
            & "                     tipomov IN ('EN','AE') AND  " _
            & "                     origen IN ('COM','INV') AND  " _
            & "                     id_emp = '" & jytsistema.WorkID & "' " _
            & "                   GROUP BY codart ) b " _
    ...
    End Function
    Public Function SeleccionMERConteo(ByVal NumeroDeConteo As String) As String
        SeleccionMERConteo = " SELECT a.conmer, date_format(a.fechacon, '%d-%m-%Y') fechacon, a.almacen, a.comentario, a.procesado, date_format(a.fechapro, '%d-%m-%Y') fechapro, " _
                & " b.codart, b.nomart, b.unidad, b.existencia, b.conteo, b.cont1, b.cont2, b.cont3, b.cont4, b.cont5, " _
    ...
    Public Function SeleccionMERServicios() As String
        SeleccionMERServicios = "select a.codser, a.desser, a.tipoiva, if( b.monto is null, 0.00, b.monto) monto, a.precio, elt(a.tiposervicio +1, 'ISLR','NORMAL') tiposervicio, a.CODCON " _
            ...
            & " left join (select fecha, tipo, monto from jsconctaiva where fecha in (select MAX(fecha) from jsconctaiva where fecha <= '" & FormatoFechaMySQL(jytsistema.sFechadeTrabajo) & "' group by tipo)) b on (a.tipoiva = b.tipo) " _
    ...
    Function SeleccionMERVentasMercanciasPLANA(ByVal TipoJerarquiaDesde As String, ByVal TipoJerarquiaHasta As String, _
        ...
        Dim str As String = ""
        str += " a.fechamov >= '" & FormatoFechaHoraMySQLInicial(FechaDesde) & "' and "
        str += " a.fechamov <= '" & FormatoFechaHoraMySQLFinal(FechaHasta) & "' and "
        ...
        Return " SELECT a.vendedor, CONCAT(c.apellidos, ' ', c.nombres) nomVendedor, " _
            ...
            & " e.descrip tipoNegocio, a.codart, b.nomart, a.numdoc, a.fechamov, a.cantidad, a.unidad, " _
    ...
    Function SeleccionMERVentasMercancias(ByVal MercanciaDesde As String, ByVal MercanciaHasta As String, ByVal OrdenadoPor As String, ByVal Operador As Integer, _
        ...
        SeleccionMERVentasMercancias = "SELECT a.*, '' codrut, '' ruta, '' codzon, '' zona FROM (SELECT ELT(b.mix+1,'ECONOMICO','ESTANDAR','SUPERIOR') AS mix, a.codart, b.NOMART, " _
                ...
                & " WHERE " & str _
                & " a.fechamov >= '" & FormatoFechaHoraMySQLInicial(FechaEmisionDesde) & "' AND " _
                & " a.fechamov <= '" & FormatoFechaHoraMySQLFinal(FechaEmisionHasta) & "' AND " _
    ...
    Function SeleccionMERVentasMercanciasAct(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        Else
            str += " b.fechamov >= '" & FormatoFechaHoraMySQLInicial(EmisionDesde) & "' and "
            str += " b.fechamov <= '" & FormatoFechaHoraMySQLFinal(EmisionHasta) & "' and "
        End If
    ...
    Public Function SeleccionMERListadoTransferencias(ByVal Desde As Date, ByVal Hasta As Date) As String

        SeleccionMERListadoTransferencias = " SELECT a.numtra, a.emision, a.alm_sale, b.desalm almacensalida," _
            & " a.alm_entra, c.desalm almacenentrada, a.comen, a.totaltra, a.totalcan, a.items, a.pesototal, a.tipo " _
            & " FROM jsmerenctra a " _
            ...
            & " WHERE " _
            & " a.emision >= '" & FormatoFechaMySQL(Desde) & "' AND " _
            & " a.emision <= '" & FormatoFechaMySQL(Hasta) & "' AND " _
    ...
    Function SeleccionMERMovimientosMercanciasResumen(ByVal CodigoDesde As String, ByVal CodigoHasta As String, _
        ...
        SeleccionMERMovimientosMercanciasResumen = "SELECT a.codart, b.nomart, b.unidad,  SUM(IF( " & strAlm & " a.tipomov = 'EN' AND origen = 'INV',  a.cantidad, 0.000) ) entradas," _
            ...
            & "                     b.id_emp = '" & jytsistema.WorkID & "' AND " _
            & "                     b.ejercicio = '" & jytsistema.WorkExercise & "' AND " _
            & "                     b.fechamov <= '" & FormatoFechaHoraMySQLFinal(FechaHasta) & "' " _
            & "                     GROUP BY b.codart, b.unidad ) a " _
            & "             LEFT JOIN jsmerequmer f ON (a.codart = f.codart AND a.unidad = f.uvalencia AND a.id_emp = f.id_emp ) " _
            & "             LEFT JOIN jsmerctainv c ON (a.codart = c.codart AND a.id_emp = c.id_emp) " _
    ...
    Function SeleccionMERComprasMercancias(ByVal MercanciaDesde As String, ByVal MercanciaHasta As String, _
        ...
        strSQL += " a.fechamov >= '" & FormatoFechaHoraMySQLInicial(EmisionDesde) & "' AND "
        strSQL += " a.fechamov <= '" & FormatoFechaHoraMySQLFinal(EmisionHasta) & "' AND "
    ...
    Function SeleccionMERComprasMercanciasAct(ByVal MyConn As MySqlConnection, ByVal lblInfo As Label, _
        ...
        Else
            strSQL += " b.fechamov >= '" & FormatoFechaHoraMySQLInicial(EmisionDesde) & "' AND "
            strSQL += " b.fechamov <= '" & FormatoFechaHoraMySQLFinal(EmisionHasta) & "' AND "
        End If
