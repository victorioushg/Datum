...
Public Class jsGenRenglonesMovimientosPlus
    ...
    Private Sub txtCodigo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtCodigo.TextChanged
        ...
        Else
            ...
            If qFound(MyConn, lblInfo, "jsmerctainv", afld, aStr) Then
                ...
                If InStr("NCV.NCC", nModulo) > 0 Then txtPorAceptaDev.Text = FormatoNumero(CDbl(qFoundAndSign(MyConn, lblInfo, "jsmerctainv", afld, aStr, "por_acepta_dev")))
    ...
    Private Sub IniciarMatrixDePrecios(CodigoDeMercancia As String)
        ...
        For iCont As Integer = 0 To 5
            aPreciosMercancias(iCont, 0) = CDbl(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " select PRECIO_" & letraPrecio(iCont) & " FROM jsmerctainv WHERE CODART = '" & CodigoDeMercancia & "' AND ID_EMP = '" & jytsistema.WorkID & "' "))
            aPreciosMercancias(iCont, 1) = CDbl(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " select DESC_" & letraPrecio(iCont) & " FROM jsmerctainv WHERE CODART = '" & CodigoDeMercancia & "' AND ID_EMP = '" & jytsistema.WorkID & "' "))
            aPreciosMercancias(iCont, 2) = CStr(EjecutarSTRSQL_Scalar(MyConn, lblInfo, " select OFERTA_" & letraPrecio(iCont) & " FROM jsmerctainv WHERE CODART = '" & CodigoDeMercancia & "' AND ID_EMP = '" & jytsistema.WorkID & "' "))
        Next
