...
Public Class jsComArcProveedoresCXP
    ...
    Private Sub IniciarCreditos(ByVal Tipo As Integer)
        ...
        HabilitarObjetos(False, True, txtDocumentoCR, txtDocSel, txtSaldoSel, txtNumPagCR, cmbNombrePago, txtNomPagCR, txtEmisionCR, txtDocSel, txtSaldoSel, _
                         txtCodigoContable)
        ...
        If Tipo <> 5 Then
            ...
            VisualizarObjetos(False, lblNotaCredito, txtCausaNotaCredito, btnCausaNotaCredito, lblDescripCausaNotaCredito)
        Else
            HabilitarObjetos(False, False, grpCRCO, optCO, optCR, txtCausaNotaCredito)
            VisualizarObjetos(True, lblNotaCredito, txtCausaNotaCredito, btnCausaNotaCredito, lblDescripCausaNotaCredito)
            txtCausaNotaCredito.Text = ""
        End If
        IniciarCajas()
        RellenaCombo(aFormaPago, cmbFPCR)

        txtCodigoContable.Text = EjecutarSTRSQL_ScalarPLUS(myConn, "SELECT codcon from jsprocatpro where codpro = '" & CodigoProveedor & "' and id_emp = '" & jytsistema.WorkID & "'  ").ToString
    ...
    Private Sub IniciarRetencionIVA()
        ...
        HabilitarObjetos(False, True, txtSaldoRetIVA, txtFechaRetIVA, txtRetIVA, txtCodigoContableIVA)
    ...
    Private Sub IniciarRetencionISLR()
        ...
        HabilitarObjetos(False, True, txtDocsSelRetISLR, txtSaldoSelRetISLR, txtFechaRetISLR, txtPorcentajeMontoBaseRetISLR, _
                        ...
                        txtAcumuladoSinRetISLR, txtSustraendoRetISLR, txtMontoRetencionISLR, _
    ...
    Private Sub IniciarDebitos(ByVal Tipo As Integer)
        ...
        HabilitarObjetos(False, True, txtDocumentoDB, txtEmisionDB, txtVenceDB, txtCodigoContableDebitos)
    ...
    Private Sub GuardarDebito(ByVal Tipo As Integer)
        ...
        InsertEditCOMPRASCXP(myConn, lblInfo, Inserta, CodigoProveedor, _
             ...
             "CXP", "", "", "", "", Documento, "0", "", jytsistema.sFechadeTrabajo, txtCodigoContableDebitos.Text, "0", _
            ...
            With lvRetISLR.Items.Item(gCont)
                ...
                If .Checked Then
                    ...
                    InsertEditCOMPRASCXP(myConn, lblInfo, Inserta, CodigoProveedor, _
                         ...
                         "CXP", "", "", "", "", Documento, "0", "", jytsistema.sFechadeTrabajo, txtCodigoContableISLR.Text, "0", _
            ...
            With lvRetIVA.Items.Item(gCont)
                If .Checked Then
                    ...
                    InsertEditCOMPRASCXP(myConn, lblInfo, Inserta, CodigoProveedor, _
                         ...
                         "CXP", "", "", "", "", Documento, "0", "", jytsistema.sFechadeTrabajo, txtCodigoContableIVA.Text, "0", _
    ...
    Private Sub ValidarCreditos(ByVal Tipo As Integer)
        ...
        If Tipo = 5 Then
            If txtCausaNotaCredito.Text.Trim = "" Then
                MensajeCritico(lblInfo, "Debe indicar una CAUSA DE CREDITO Válida...")
                Exit Sub
            Else
                If CBool(EjecutarSTRSQL_ScalarPLUS(myConn, " select VALIDA_DOCUMENTOS from jsconcausas_notascredito where codigo = '" & txtCausaNotaCredito.Text & "' and origen = 'CXP' ")) Then
                ...
                Else
                    If ValorEntero(txtDocSel.Text) <> 0 Then
                        MensajeCritico(lblInfo, "NO DEBE seleccionar documentos para esta causa de NOTA CREDITO...")
                        Exit Sub
                    End If
                End If
            End If
        ...
        If IsNumeric(txtImporteCR.Text) Then
            If ValorNumero(txtImporteCR.Text) > 0 Then
                MensajeCritico(lblInfo, "Debe indicar un importe MENOR O IGUAL a cero (0)")
                Exit Sub
            Else
                ...
                Select Case cmbTipo.SelectedIndex
                    ...
                    Case 5 'NOTAS CREDITO
                        If ValorNumero(txtImporteCR.Text) = 0 Then
                            MensajeCritico(lblInfo, "Debe indicar un importe MENOR QUE CERO")
                            Exit Sub
                        End If
                End Select
        ...
        If CBool(ParametroPlus(myConn, Gestion.iCompras, "COMPARAM05")) Then
            If txtCodigoContable.Text.Trim = "" Then
                MensajeCritico(lblInfo, "Debe indicar CODIGO CONTABLE ...")
                Exit Sub
            End If
        End If
    ...
    Private Sub GuardarCreditos(ByVal Tipo As Integer)
            ...
            Select Case cmbFPCR.SelectedIndex
                Case 0, 3 'EF, CT
                    ...
                    InsertEditBANCOSRenglonCaja(myConn, lblInfo, True, cmbCajaCR.SelectedValue, UltimoCajaMasUno(myConn, lblInfo, cmbCajaCR.SelectedValue), _
                        ...
                        txtNumPagCR.Text, NombreDePago, ValorNumero(txtImporteCR.Text), txtCodigoContable.Text, txtConceptoCR.Text, "", jytsistema.sFechadeTrabajo, 1, _
            ...
            If lv.Items(jCont).Checked Then
                ...
                If MontoACancelar < 0 Then 'FC,GR,ND
                    If MontoNegativo > 0 Then
                        ...
                        InsertEditCOMPRASCancelacion(myConn, lblInfo, True, CodigoProveedor, lv.Items(jCont).SubItems(1).Text, _
                                                    NumeroDocumento, CDate(lv.Items(jCont).SubItems(3).Text), txtReferenciaCR.Text, _
                        ...
                        If TipoDocumento = "FC" Or TipoDocumento = "GR" Then
                            ...
                            If Math.Abs(MontoACancelar) > MontoNegativo Then
                                InsertEditCOMPRASCXP(myConn, lblInfo, True, CodigoProveedor, IIf(Tipo = 3 OrElse Tipo = 4, "AB", "NC"), NumeroDocumento, _
                                    ...
                                    txtBancoDeposito.Text, cmbCajaCR.SelectedValue, Documento, IIf(CInt(txtDocSel.Text) <= 1, "0", "1"), causaNotaCredito, jytsistema.sFechadeTrabajo, _
                                    txtCodigoContable.Text, "", lv.Items(jCont).SubItems(1).Text, 0.0, 0.0, Documento, "", "", Remesa, "", "", TipoProveedor, FOTipo.Credito, "0")


                            Else

                                InsertEditCOMPRASCXP(myConn, lblInfo, True, CodigoProveedor, aTipoNick(Tipo), NumeroDocumento, _
                                    CDate(txtEmisionCR.Text), FormatoHora(Now), CDate(txtEmisionCR.Text), _
                        ...
                        Else
                            ...
                            If Math.Abs(MontoACancelar) > MontoNegativo Then
                                ...
                                InsertEditCOMPRASCXP(myConn, lblInfo, True, CodigoProveedor, "NC", NumeroDocumento, _
                                            CDate(txtEmisionCR.Text), FormatoHora(Now()), CDate(txtEmisionCR.Text), _
                                            txtReferenciaCR.Text, txtConceptoCR.Text, Math.Abs(MontoNegativo), 0.0, aFormaPagoAbreviada(cmbFPCR.SelectedIndex), _
                                            txtNumPagCR.Text, NombreDePago, txtBeneficiario.Text, "CXP", txtNumeroDeposito.Text, txtCuentaDeposito.Text, _
                                            txtBancoDeposito.Text, cmbCajaCR.SelectedValue, Documento, IIf(CInt(txtDocSel.Text) <= 1, "0", "1"), causaNotaCredito, jytsistema.sFechadeTrabajo, _
                ...
                ElseIf MontoACancelar > 0 Then
                    ...
                    If Math.Abs(MontoACancelar) > MontoPositivo Then
                        ...
                        InsertEditCOMPRASCXP(myConn, lblInfo, True, CodigoProveedor, "ND", NumeroDocumento, _
                            CDate(txtEmisionCR.Text), FormatoHora(Now()), CDate(txtEmisionCR.Text), _
                            ...
                            txtCodigoContable.Text, "", lv.Items(jCont).SubItems(1).Text, 0.0, 0.0, Documento, "", "", Remesa, "", "", _
        ...
        If Resto > 0 Then
            ...
            InsertEditCOMPRASCXP(myConn, lblInfo, True, CodigoProveedor, "NC", NumeroDocumento, _
                           ...
                           cmbCajaCR.SelectedValue, Documento, sDocSel, causaNotaCredito, jytsistema.sFechadeTrabajo, _
                           txtCodigoContable.Text, "", sDocCan, 0.0, 0.0, Documento, "", "", Remesa, "", "", _
    ...
    Private Sub txtCausaNotaCredito_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtCausaNotaCredito.TextChanged
        lblDescripCausaNotaCredito.Text = EjecutarSTRSQL_ScalarPLUS(myConn, " select descripcion from jsconcausas_notascredito where codigo = '" & txtCausaNotaCredito.Text & "' and origen = 'CXC' ")
        txtConceptoCR.Text += " " + lblDescripCausaNotaCredito.Text
        txtReferenciaCR.Text = ContadorNC_Financiera(myConn, lblInfo, txtCausaNotaCredito.Text, "CXP")
        causaNotaCredito = EjecutarSTRSQL_ScalarPLUS(myConn, " select NOMPAG from jsconcausas_notascredito where codigo = '" & txtCausaNotaCredito.Text & "' and origen = 'CXC' ")

        If CBool(EjecutarSTRSQL_ScalarPLUS(myConn, " select CR from jsconcausas_notascredito where codigo = '" & txtCausaNotaCredito.Text & "' and origen = 'CXC' ")) Then
            optCR.Checked = True
