...
Public Class jsVenArcClientes
    ...
    Private Sub AsignarTooltips()
        ...
        C1SuperTooltip1.SetToolTip(btnRIF, "Consulta RIF desde la página WEB del <B>SENIAT</B> ")
        C1SuperTooltip1.SetToolTip(btnSADA, "Consulta códigos desde la página WEB del <B>SADA</B> ")
        C1SuperTooltip1.SetToolTip(btnTablaSADA, "Consulta Tabla de códigos SADA del <B>Cliente</B> ")
        ...
        If dt.Rows.Count > 0 Then
            ...
            With dt
                ...
                With .Rows(nRow)
                    ...
                    If InStr(MuestraCampoTexto(.Item("forultcobro")), ".EF.CH.CT.DP.TR.TA") > 0 Then
    ...
    Private Sub IniciarCliente(ByVal Inicio As Boolean)
        ...
        txtDescA.Text = FormatoNumero(CDbl(vDesc1(2).ToString))
        txtDescB.Text = FormatoNumero(CDbl(vDesc2(2).ToString))
        txtDescC.Text = FormatoNumero(CDbl(vDesc3(2).ToString))
        txtDescD.Text = FormatoNumero(CDbl(vDesc4(2).ToString))

        txtDeA.Text = FormatoEntero(CInt(vDesc1(0).ToString))
        txtDeB.Text = FormatoEntero(CInt(vDesc2(0).ToString))
        txtDeC.Text = FormatoEntero(CInt(vDesc3(0).ToString))
        txtDeD.Text = FormatoEntero(CInt(vDesc4(0).ToString))

        txtAA.Text = FormatoEntero(CInt(vDesc1(1).ToString))
        txtAB.Text = FormatoEntero(CInt(vDesc2(1).ToString))
        txtAC.Text = FormatoEntero(CInt(vDesc3(1).ToString))
        txtAD.Text = FormatoEntero(CInt(vDesc4(1).ToString))
    ...
    Private Function Validado() As Boolean
                ...
                If EsRIF(txtRIF.Text) Then
                    ...
                    If rRif <> "0" Then
                        If CStr(EjecutarSTRSQL_Scalar(myConn, lblInfo, " select ALTERNO from jsvencatcli where RIF = '" & txtRIF.Text & "' AND ALTERNO = '" & txtAlterno.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")) <> "0" Then
                            MensajeCritico(lblInfo, "EXISTE CLIENTE CON EL MISMO RIF E IGUAL CODIGO ALTERNO. VERIFIQUE POR FAVOR...")
        ...
        If NivelUsuario(myConn, lblInfo, jytsistema.sUsuario) = 0 AndAlso _
            ...
            MensajeCritico(lblInfo, "NO POSEE PERMISOS SUFICIENTES PARA QUE ESTE CLIENTE FACTURE DESDE EL COSTO ...")
            Exit Function
    ...
    Private Sub GuardarTXT()
        ...
        Else
            ...
            If RutaDesincorporado.Trim() <> "" Then
                Dim AsesorDesincorporado As String = EjecutarSTRSQL_Scalar(myConn, lblInfo, " SELECT codven FROM jsvenencrut WHERE CODRUT = '" & RutaDesincorporado & "' AND tipo = 0 AND id_emp = '" & jytsistema.WorkID & "' ")
                Dim NumeroDesincorporadoEnRuta As Integer = CInt(EjecutarSTRSQL_Scalar(myConn, lblInfo, " SELECT items FROM jsvenencrut WHERE CODRUT = '" & RutaDesincorporado & "' AND tipo = 0 AND id_emp = '" & jytsistema.WorkID & "' ")) + 1
                Dim CondicionDesincorporadoRuta As Integer = CInt(EjecutarSTRSQL_Scalar(myConn, lblInfo, " SELECT condicion FROM jsvenencrut WHERE CODRUT = '" & RutaDesincorporado & "' AND tipo = 0 AND id_emp = '" & jytsistema.WorkID & "' "))

                EjecutarSTRSQL_Scalar(myConn, lblInfo, " update jsvenrenrut set codrut = '" & RutaDesincorporado & "', numero = " & NumeroDesincorporadoEnRuta & " " _
                                      & " where " _
                                      & " cliente = '" & txtCodigo.Text & "' and " _
                                      & " tipo = '0' and " _
                                      & " id_emp = '" & jytsistema.WorkID & "' ")

                EjecutarSTRSQL(myConn, lblInfo, " update jsvenencrut set  items = " & NumeroDesincorporadoEnRuta & " where codrut = '" & RutaDesincorporado & "' and id_emp = '" & jytsistema.WorkID & "' ")

                EjecutarSTRSQL(myConn, lblInfo, " update jsvencatcli set vendedor = '" & AsesorDesincorporado & "', cobrador = '" & AsesorDesincorporado _
    ...
    Private Sub AsignarExpediente(ByVal nCodigoCliente As String)
        ...
        Dim aAncexp() As Long = {180, 210, 90, 350}
    ...
    '    If cmbFacturaDesde.Items.Count >= 2 Then
    ...
    '        cmbFacturaDesde.SelectedIndex = IIf(cmbFacturaDesde.SelectedIndex = 0, 1, 0)
    '        'End If
    '    End If
    ...
    Private Sub cmbEstatus_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cmbEstatus.SelectedIndexChanged
        Select Case cmbEstatus.SelectedIndex
            Case 3 'DESINCORPORADOS
                MensajeInformativoPlus("HA ESCOGIDO DESINCORPORAR CLIENTE POR LO TANTO. SE CAMBIARA DE RUTA A LA RUTA ")
        End Select
    End Sub
    ...
    Private Sub btnRIF_Click(sender As System.Object, e As System.EventArgs) Handles btnRIF.Click
        If EsRIF(txtRIF.Text) Then
            If validarRif(txtRIF.Text) Then
                ...
                f = Nothing
            Else
                MensajeCritico(lblInfo, "RIF IVALIDO!!!!. VERIFIQUE POR FAVOR")
            End If
        Else
    ...
    Private Sub btnSADA_Click(sender As System.Object, e As System.EventArgs) Handles btnSADA.Click
        If EsRIF(txtRIF.Text) Then
            If validarRif(txtRIF.Text) Then
                Dim f As New jsGenConsultaRIF
                'f.DireccionURL = "https://sistema.sada.gob.ve/empresa_nueva.php"
