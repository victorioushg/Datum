...
Public Class jsVenArcClientesCXCPlus
    ...
    Private Sub IniciarRetencionIVA()
        ...
        HabilitarObjetos(False, True, txtTotalRetIVA, txtFechaComprobanteRetIVA, txtFechaRecepcionRetIVA, txtAsesorRetIVA)
    ...
    Private Sub CalculaTotalRetIVA()
        ...
        For Each selectedItem As DataGridViewRow In dgRetIVA.Rows
            ...
            End If
        Next
        txtTotalRetIVA.Text = FormatoNumero(montoSeleccionado)
    End Sub
    ...
    Private Sub IniciarSaldos(ByVal lv As ListView)
        ...
        Dim strSaldos As String = " SELECT a.codcli, b.nombre, a.nummov, a.tipomov, a.refer, a.emision, a.vence, a.importe, c.saldo, a.codven " _
                                  ...
                                  & " WHERE " _
                                  & " CONCAT(a.nummov, a.emision, a.hora) IN (SELECT MIN(CONCAT(nummov, emision, hora)) FROM jsventracob a WHERE a.codcli = '" & CodigoCliente & "' AND a.id_emp = '" & jytsistema.WorkID & "'  GROUP BY nummov) AND " _
                                  & " a.codcli = '" & CodigoCliente & "'  AND " _
                                  & " a.codcli <> '' AND " _
    ...
    Private Sub IniciarSaldosIVA(PorcentajeRetencionIVA As Double)
        ...
        Dim tblSaldosRetIVA As String = "tblSaldosIVA" & NumeroAleatorio(10000)
        Dim aFields() As String = {"sel.entero", "codcli.cadena15", "nombre.cadena250", "nummov.cadena20", "tipomov.cadena2", _
                                       "num_control.cadena20", "emision.fecha", "vence.fecha", "importe.doble19", "impiva.doble19", _
                                       "retimpiva.doble19"}
        ...
        '                          & " UNION " _
        '                          & " SELECT 0 sel, a.codcli, b.nombre, a.nummov, a.tipomov, d.num_control, a.emision, a.vence, a.importe, a.importe/ABS(a.importe)*c.impiva impiva, 0.00 retimpiva " _
        '                          & " FROM jsventracob a " _
        '                          & " LEFT JOIN jsvencatcli b ON (a.codcli = b.codcli AND a.id_emp = b.id_emp) " _
        ...
        EjecutarSTRSQL(myConn, lblInfo, " INSERT INTO " & tblSaldosRetIVA _
    ...
    Private Sub GuardarRetencionIVA()
        ...
        For Each selectedItem As DataGridViewRow In dgRetIVA.Rows
            If selectedItem.Cells(0).Value Then
                With selectedItem
                    InsertEditVENTASCXC(myConn, lblInfo, Inserta, CodigoCliente, _
                          IIf(.Cells(2).Value = "NC", "ND", "NC"), .Cells(1).Value, CDate(txtFechaRecepcionRetIVA.Text), FormatoHora(Now()), CDate(txtFechaRecepcionRetIVA.Text), _
    ...
    Private Sub cmbFPCR_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbFPCR.SelectedIndexChanged
        ...
        'txtEmisionCR.Text = FormatoFecha(jytsistema.sFechadeTrabajo)
    ...
    Private Sub dgRetIVA_CellValidating(ByVal sender As Object, ByVal e As DataGridViewCellValidatingEventArgs) _
        ...
        If Not IsNumeric(eSinComas) Then
    ...
    Private Sub dgRetIVA_DataError(ByVal sender As Object, ByVal e As DataGridViewDataErrorEventArgs) _
        ...
        If StrComp(e.Exception.Message, "Input string was not in a correct format.") = 0 Or _
            ...
            MensajeAdvertencia(lblInfo, "Por favor indique un número sin formato (sin comas)")
