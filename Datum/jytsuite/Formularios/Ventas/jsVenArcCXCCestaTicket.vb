...
Public Class jsVenArcCXCCestaTicket
    ...
    Public Sub Cargar(ByVal MyCon As MySqlConnection, ByVal NumeroCancelacion As String, _
        ...
        txt0.Text = ft.FormatoNumero(0.0)
        txt1.Text = ft.FormatoNumero(0.0)
        txt2.Text = ft.FormatoNumero(0.0)
        txt3.Text = ft.FormatoNumero(MontoACancelar)

        If dtCorredores.Rows.Count <= 0 Then ft.habilitarObjetos(False, True, txtBarraticket)
    ...
    Private Sub btnCancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCancel.Click
        ...
        ft.Ejecutar_strSQL(myconn, "DELETE FROM jsventabtic where NUMCAN = '" & numCancelacion & "' AND ID_EMP = '" & jytsistema.WorkID & "' ")
        Me.Close()
    ...
    Private Function Validado() As Boolean
        ...
        '    ft.MensajeCritico( "LA SUMA DE LOS TICKETES DEBE SER MENOR O IGUAL AL MONTO DE FACTURAS A CANCELAR...")
        '    Exit Function
        'End If
        Return True
    ...
    Private Sub btnEliminar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEliminar.Click
        ...
        If dtTickets.Rows.Count > 0 Then
            Respuesta = MsgBox(" ¿ Está seguro que desea eliminar el ticket n° >>>>  " & dtTickets.Rows(Me.BindingContext(dsLocal, nTablaTickets).Position).Item("ticket") & " <<<< ?", vbYesNo, "Eliminar registro ... ")
            If Respuesta = vbYes Then
                ft.Ejecutar_strSQL(myconn, " delete from jsventabtic where TICKET = '" & dtTickets.Rows(Me.BindingContext(dsLocal, nTablaTickets).Position).Item("ticket") & "'")
                CalculaTotales()
            End If
        End If
        txtBarraticket.Enabled = True
        ft.enfocarTexto(txtBarraticket)
    End Sub

    Private Sub CalculaTotales()

        txt0.Text = ft.FormatoNumero(ft.DevuelveScalarDoble(MyConn, "select sum(monto) from jsventabtic WHERE " _
                                                                        & " ID_EMP = '" & jytsistema.WorkID & "' AND" _
                                                                        & " NUMCAN = '" & numCancelacion & "' GROUP BY NUMCAN"))

        txt1.Text = ft.FormatoNumero(ft.DevuelveScalarDoble(MyConn, "select sum(comision)  from jsventabtic WHERE " _
                                                                        & " ID_EMP = '" & jytsistema.WorkID & "' AND" _
                                                                        & " NUMCAN = '" & numCancelacion & "' GROUP BY NUMCAN"))

        txt2.Text = ft.FormatoNumero(ValorNumero(txt0.Text) - ValorNumero(txt1.Text))
    ...
    Private Sub IniciaResumen()
        ...
        strSQLResumen = "select CORREDOR, IFNULL(COUNT(*),0) AS CANTIDAD, SUM(MONTO) AS IMPORTE from jsventabtic WHERE " _
                                    & " ID_EMP = '" & jytsistema.WorkID & "' AND " _
        ...
        Dim aAncR() As Integer = {70, 100, 100}
    ...
    Private Sub IniciaTickets()
        ...
        Dim aAnc() As Integer = {350, 70, 100, 100}
    ...
    Private Sub txtBarraticket_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtBarraticket.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            lenCodigoBarraCorredor = ft.DevuelveScalarEntero(MyConn, " select lencodbar from jsvencestic where codigo = '" & CodigoCorredor & "' and id_emp = '" & jytsistema.WorkID & "' ")
            iniPrecioEnBarraCorredor = ft.DevuelveScalarEntero(MyConn, " select inicioprecio from jsvencestic where codigo = '" & CodigoCorredor & "' and id_emp = '" & jytsistema.WorkID & "' ")
            lenPrecioEnBarraCorredor = ft.DevuelveScalarEntero(MyConn, " select lenprecio from jsvencestic where codigo = '" & CodigoCorredor & "' and id_emp = '" & jytsistema.WorkID & "' ")
            iniTipoCorredor = ft.DevuelveScalarEntero(MyConn, " select iniciotipo from jsvencestic where codigo = '" & CodigoCorredor & "' and id_emp = '" & jytsistema.WorkID & "' ")
            lenTipoCorredor = ft.DevuelveScalarEntero(MyConn, " select lentipo from jsvencestic where codigo = '" & CodigoCorredor & "' and id_emp = '" & jytsistema.WorkID & "' ")

            Dim lenBarra As Integer = txtBarraticket.Text.Length
            txtBarraticket.Enabled = False

            If lenBarra <> lenCodigoBarraCorredor Then
                ft.MensajeCritico(" La longitud del Código de barra de corredor escaneado ES DIFERENTE al INDICADO PARA ESTE CORREDOR ...")
                txtBarraticket.Enabled = True
                ft.enfocarTexto(txtBarraticket)
                Exit Sub
            End If
            ...
            Dim perComisionProveedor As Double = ft.DevuelveScalarDoble(MyConn, " select com_corredor from jsvencestip where corredor = '" & CodigoCorredor & "' and tipo = '" & TipoTicket & "' and id_emp = '" & jytsistema.WorkID & "'  ")

            If ft.DevuelveScalarEntero(MyConn, "SELECT COUNT(*) FROM jsventabtic WHERE TICKET = '" & txtBarraticket.Text & "'AND CORREDOR = '" & CodigoCorredor & "' ") = 0 Then
                ...
                If ft.DevuelveScalarEntero(MyConn, " select COUNT(*) from jsvenvaltic where " _
                    ...
                    ft.mensajeCritico(":-)) DEBE LEER ESTE TICKET DE NUEVO ooooooooo")

                Else 'Valor Existe en BD
                    InsertaTicket = True
                End If
                If InsertaTicket Then
                    Dim ValorTicket As Double = ft.DevuelveScalarDoble(MyConn, " select valor from jsvenvaltic where " _
                                                                           & " codigo = '" & CodigoCorredor & "' and " _
                    ...
                    ft.Ejecutar_strSQL(MyConn, " lock tables jsventabtic write ")
                    ft.Ejecutar_strSQL(MyConn, "INSERT INTO jsventabtic " _
                    ...
                    ft.Ejecutar_strSQL(MyConn, " unlock tables ")
                ...
                End If
            Else
                ft.mensajeCritico("CODIGO DE BARRA YA EXISTE. VERIFIQUE POR FAVOR...")
            End If
            txtBarraticket.Text = ""
            txtBarraticket.Enabled = True
            ft.enfocarTexto(txtBarraticket)
    ...
    Private Function PorcentajeComisionCliente(ByVal MyConn As MySqlConnection, ByVal Corredor As String, ByVal Tipo As String) As Double
        ...
        PorcentajeComisionCliente = ft.DevuelveScalarDoble(MyConn, " select porcomcli from jsvencestic where codigo = '" & Corredor & "' and id_emp = '" & jytsistema.WorkID & "' ")
