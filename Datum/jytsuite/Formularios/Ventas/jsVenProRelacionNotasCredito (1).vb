...
Public Class jsVenProRelacionNotasCredito
    ...
    Private Sub IniciarDocumento(ByVal Inicio As Boolean)
        ...
        If Inicio Then
            txtCodigo.Text = "RNCTMP" & NumeroAleatorio(100000)
        Else
            txtCodigo.Text = ""
        End If
    ...
    Private Sub AgregaYCancela()
        ...
        EjecutarSTRSQL(myConn, lblInfo, " delete from jsvenrenrel where codigoguia = '" & txtCodigo.Text & "' and tipo = 1 and id_emp = '" & jytsistema.WorkID & "' ")
    ...
    Private Sub GuardarTXT()
        ...
        If i_modo = movimiento.iAgregar Then
            ...
            Codigo = AutoCodigo(8, ds, nTabla, "CODIGOGUIA")  ' Contador(myConn, lblInfo, Gestion.iVentas, "VENNUMREL", "16")

            EjecutarSTRSQL(myConn, lblInfo, " update jsvenrenrel set codigoguia = '" & Codigo & "' where codigoguia = '" & txtCodigo.Text & "' and tipo = '1' and  id_emp = '" & jytsistema.WorkID & "' ")
        End If
    ...
    Private Sub CalculaTotales()
        ...
        txtTotal.Text = FormatoNumero(CDbl(EjecutarSTRSQL_Scalar(myConn, lblInfo, " select sum(totalfac) from jsvenrenrel where codigoguia = '" & txtCodigo.Text & "' and tipo = 1 and id_emp = '" & jytsistema.WorkID & "' group by codigoguia ")))
        tslblPesoT.Text = FormatoCantidad(CDbl(EjecutarSTRSQL_Scalar(myConn, lblInfo, " select sum(kilosfac) from jsvenrenrel where codigoguia = '" & txtCodigo.Text & "' and tipo = 1 and id_emp = '" & jytsistema.WorkID & "' group by codigoguia ")))
        ...
        If nPosicionRenglon >= 0 Then
            ...
            If sRespuesta = MsgBoxResult.Yes Then
                With dtRenglones.Rows(nPosicionRenglon)
                    ...
                    EjecutarSTRSQL(myConn, lblInfo, " update jsvenencncr set relncr = '' where " _
    ...
    Private Sub ImpresiónNotasDeCréditoToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles ImpresiónNotasDeCréditoToolStripMenuItem.Click
        Dim sRespuesta As Microsoft.VisualBasic.MsgBoxResult
        sRespuesta = MsgBox(" ¿ Esta Seguro que desea imprimir Notas de Crédito de Relación N° " & txtCodigo.Text & " ?", MsgBoxStyle.YesNo, sModulo & " Impresión Notas de Crédito Relación ... ")
