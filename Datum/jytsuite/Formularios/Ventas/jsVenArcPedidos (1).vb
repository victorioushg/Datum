...
Public Class jsVenArcPedidos
    ...
    Private strSQL As String = " (select a.*, b.nombre from jsvenencped a " _
            ...
            & " where " _
            & " a.emision >= '" & FormatoFechaMySQL(DateAdd("m", -MesesAtras.i2, jytsistema.sFechadeTrabajo)) & "' and " _
    ...
    Private Sub AbrirIVA(ByVal NumeroDocumento As String)
        ...
        strSQLIVA = "select * from jsvenivaped " _
                            & " where " _
        ...
        txtTotalIVA.Text = FormatoNumero(CDbl(EjecutarSTRSQL_Scalar(myConn, lblInfo, " select sum(impiva) from jsvenivaped where numped = '" & txtCodigo.Text & "' and id_emp = '" & jytsistema.WorkID & "' group by numped ")))

    End Sub
    Private Sub AbrirDescuentos(ByVal NumeroDocumento As String)

        strSQLDescuentos = "select * from jsvendesped " _
                            & " where " _
                            & " numped  = '" & NumeroDocumento & "' and " _
                            & " ejercicio = '" & jytsistema.WorkExercise & "' and " _
                            & " id_emp = '" & jytsistema.WorkID & "' order by renglon "
        ...
        MontoParaDescuento = MontoParaDescuentoVentas(myConn, lblInfo, "numped", "jsvenrenped", txtCodigo.Text)

        txtDescuentos.Text = FormatoNumero(CDbl(EjecutarSTRSQL_Scalar(myConn, lblInfo, " select sum(descuento) from jsvendesped where numped = '" & txtCodigo.Text & "' and id_emp = '" & jytsistema.WorkID & "' group by numped ")))
    ...
    Private Sub IniciarDocumento(ByVal Inicio As Boolean)
        ...
        If Inicio Then
            txtCodigo.Text = "PETMP" & NumeroAleatorio(100000)
        Else
            txtCodigo.Text = ""
        End If
    ...
    Private Sub AgregaYCancela()
        ...
        EjecutarSTRSQL(myConn, lblInfo, " delete from jsvenrenped where numped = '" & txtCodigo.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")
        EjecutarSTRSQL(myConn, lblInfo, " delete from jsvenivaped where numped = '" & txtCodigo.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")
        EjecutarSTRSQL(myConn, lblInfo, " delete from jsvendesped where numped = '" & txtCodigo.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")
        EjecutarSTRSQL(myConn, lblInfo, " delete from jsvenrencom where numdoc = '" & txtCodigo.Text & "' and origen = 'PED' and id_emp = '" & jytsistema.WorkID & "' ")
    ...
    Private Function Validado() As Boolean
        ...
        If Disponible < 0 Then
            MensajeCritico(lblInfo, "Este pedido excede la disponibilidad...")
            Return False
        End If
    ...
    Private Sub GuardarTXT()
        ...
        If i_modo = movimiento.iAgregar Then
            ...
            EjecutarSTRSQL(myConn, lblInfo, " update jsvenrenped set numped = '" & Codigo & "' where numped = '" & txtCodigo.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")
            EjecutarSTRSQL(myConn, lblInfo, " update jsvenivaped set numped = '" & Codigo & "' where numped = '" & txtCodigo.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")
            EjecutarSTRSQL(myConn, lblInfo, " update jsvendesped set numped = '" & Codigo & "' where numped = '" & txtCodigo.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")
            EjecutarSTRSQL(myConn, lblInfo, " update jsvenrencom set numdoc = '" & Codigo & "' where numdoc = '" & txtCodigo.Text & "' and origen = 'PED' and id_emp = '" & jytsistema.WorkID & "' ")

        End If

        InsertEditVENTASEncabezadoPedido(myConn, lblInfo, Inserta, Codigo, CDate(txtEmision.Text), CDate(txtEntrega.Text), _
    ...
    Private Sub btnEliminar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEliminar.Click
        If CBool(ParametroPlus(MyConn, Gestion.iVentas, "VENPEDPA01")) Then
            ...
            If sRespuesta = MsgBoxResult.Yes Then
                ...
                EjecutarSTRSQL(myConn, lblInfo, " delete from jsvenencped where " _
    ...
    Private Sub CalculaTotales()
        ...
        txtSubTotal.Text = FormatoNumero(CalculaTotalRenglonesVentas(myConn, lblInfo, "jsvenrenped", "numped", "totren", txtCodigo.Text, 0))
        AbrirDescuentos(txtCodigo.Text)
        txtCargos.Text = FormatoNumero(CalculaTotalRenglonesVentas(myConn, lblInfo, "jsvenrenped", "numped", "totren", txtCodigo.Text, 1))
        CalculaDescuentoEnRenglones()
        CalculaTotalIVAVentas(myConn, lblInfo, "jsvendesped", "jsvenivaped", "jsvenrenped", "numped", txtCodigo.Text, "impiva", "totrendes", CDate(txtEmision.Text), "totren")
        AbrirIVA(txtCodigo.Text)
        ...
        tslblPesoT.Text = FormatoCantidad(CalculaPesoDocumento(myConn, lblInfo, "jsvenrenped", "peso", "numped", txtCodigo.Text))

    End Sub
    Private Sub CalculaDescuentoEnRenglones()

        EjecutarSTRSQL_Scalar(myConn, lblInfo, " update jsvenrenped set totrendes = totren - totren * " & ValorNumero(txtDescuentos.Text) / IIf(ValorNumero(txtSubTotal.Text) > 0, ValorNumero(txtSubTotal.Text), 1) & " " _
            & " where " _
            & " numped = '" & txtCodigo.Text & "' and " _
            & " renglon > '' and " _
            & " item > '' and " _
    ...
    Private Sub btnAgregarMovimiento_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregarMovimiento.Click
        If txtNombreCliente.Text <> "" Then
            ...
            f.Agregar(myConn, ds, dtRenglones, "PED", txtCodigo.Text, CDate(txtEmision.Text), , cmbTarifa.Text, txtCliente.Text, , , , , , , , , , txtAsesor.Text)
            ds = DataSetRequery(ds, strSQLMov, myConn, nTablaRenglones, lblInfo)
    ...
    Private Sub btnEditarMovimiento_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEditarMovimiento.Click
        ...
        If dtRenglones.Rows.Count > 0 Then
            Dim f As New jsGenRenglonesMovimientos
            f.Apuntador = Me.BindingContext(ds, nTablaRenglones).Position
            f.Editar(myConn, ds, dtRenglones, "PED", txtCodigo.Text, CDate(txtEmision.Text), , cmbTarifa.Text, txtCliente.Text, _
        ...
        If nPosicionRenglon >= 0 Then
            ...
            If sRespuesta = MsgBoxResult.Yes Then
                With dtRenglones.Rows(nPosicionRenglon)
                    ...
                    nPosicionRenglon = EliminarRegistros(myConn, lblInfo, ds, nTablaRenglones, "jsvenrenped", strSQLMov, aCamposDel, aStringsDel, _
    ...
    Private Sub btnImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimir.Click
        ...
        If DocumentoImpreso(myConn, lblInfo, "jsvenencped", "numped", txtCodigo.Text) AndAlso _
        ...
        f.Cargar(TipoCargaFormulario.iShowDialog, ReporteVentas.cpEDIDO, "Pedido", txtCliente.Text, txtCodigo.Text, CDate(txtEmision.Text))
        f = Nothing

    End Sub

    Private Sub btnEmision_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEmision.Click
        If CBool(ParametroPlus(MyConn, Gestion.iVentas, "VENPEDPA03")) Then
    ...
    Private Sub txtCliente_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtCliente.TextChanged
        If txtCliente.Text <> "" Then
            ...
            If i_modo = movimiento.iAgregar Then
                ...
                FechaVencimiento = FechaVencimientoFactura(myConn, txtCliente.Text, CDate(txtEmision.Text))
    ...
    Private Sub btnAgregaDescuento_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregaDescuento.Click
        If txtCliente.Text.Trim <> "" Then
            ...
            Else
                ...
                f.Agregar(myConn, ds, dtDescuentos, "jsvendesped", "numped", txtCodigo.Text, sModulo, txtAsesor.Text, 0, CDate(txtEmision.Text), ValorNumero(txtSubTotal.Text))
                CalculaTotales()
                f = Nothing
            End If
        End If
    ...
    Private Sub btnEliminaDescuento_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEliminaDescuento.Click
        If nPosicionDescuento >= 0 Then
            With dtDescuentos.Rows(nPosicionDescuento)
                ...
                nPosicionDescuento = EliminarRegistros(myConn, lblInfo, ds, nTablaDescuentos, "jsvendesped", strSQLDescuentos, aCamposDel, aStringsDel, _
    ...
    Private Sub btnAgregarServicio_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregarServicio.Click
        If txtNombreCliente.Text <> "" Then
            Dim f As New jsGenRenglonesMovimientos
            f.Apuntador = Me.BindingContext(ds, nTablaRenglones).Position
            f.Agregar(myConn, ds, dtRenglones, "PED", txtCodigo.Text, CDate(txtEmision.Text), , cmbTarifa.Text, txtCliente.Text, True, , , , , , , , , txtAsesor.Text)
            ds = DataSetRequery(ds, strSQLMov, myConn, nTablaRenglones, lblInfo)
            AsignaMov(f.Apuntador, True)
            CalculaTotales()
            f = Nothing
        ...
        If dtRenglones.Rows.Count > 0 Then
            ...
            With dtRenglones.Rows(nPosicionRenglon)
                ...
                If f.NuevoPrecio <> PrecioAnterior Then
                    EjecutarSTRSQL(myConn, lblInfo, " update jsvenrenped " _
