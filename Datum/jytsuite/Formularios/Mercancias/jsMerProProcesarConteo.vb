...
Public Class jsMerProPROCESARConteo
    ...
    Public Sub Cargar(ByVal MyCon As MySqlConnection)
        ...
        ft.visualizarObjetos(False, Label2, txtJerarquiaDesde, btnJerarquiaDesde)
        Me.Show()
    ...
    Private Sub Reprocesar()
        ...
        If dt.Rows.Count > 0 Then
            For hCont = 0 To dt.Rows.Count - 1
                With dt.Rows(hCont)
                    ...
                    Dim FechaAsiento As Date = ft.DevuelveScalarFecha(myConn, " select fechacon from jsmerenccon where conmer = '" & txtCategoriaDesde.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")
                    Dim Almacen As String = ft.DevuelveScalarCadena(myConn, " select almacen from jsmerenccon where conmer = '" & txtCategoriaDesde.Text & "' and id_emp = '" & jytsistema.WorkID & "' ")
                    If Almacen = "" Then Almacen = "00001"
                    Dim PesoArticulo As Double = ft.DevuelveScalarDoble(myConn, "select pesounidad from jsmerctainv where codart = '" & .Item("codart") & "' and id_emp = '" & jytsistema.WorkID & "' ")

                    Dim CantidadAjuste As Double = .Item("existencia") - .Item("conteo")
        ...
        ft.Ejecutar_strSQL(myconn, " update jsmerenccon set procesado = 1, fechapro = '" & ft.FormatoFechaMySQL(jytsistema.sFechadeTrabajo) & _
