...
Public Class jsMerArcComponentesCombo
    ...
    Private Sub jsMerArcComponentesCombo_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ft.habilitarObjetos(False, True, txtCostos, txtPesos)
    End Sub
        ...
        For Each nRow As DataRow In dt.Rows
            With nRow
                ...
                If .Item("CODARTCOM").ToString.Substring(0, 1) = "$" Then
                    precioTotalA += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio from jsmercatser where codser = '" & .Item("codartcom").ToString.Substring(1) & "' and id_emp = '" & jytsistema.WorkID & "' ")
                    precioTotalB += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio from jsmercatser where codser = '" & .Item("codartcom").ToString.Substring(1) & "' and id_emp = '" & jytsistema.WorkID & "' ")
                    precioTotalC += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio from jsmercatser where codser = '" & .Item("codartcom").ToString.Substring(1) & "' and id_emp = '" & jytsistema.WorkID & "' ")
                    precioTotalD += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio from jsmercatser where codser = '" & .Item("codartcom").ToString.Substring(1) & "' and id_emp = '" & jytsistema.WorkID & "' ")
                    precioTotalE += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio from jsmercatser where codser = '" & .Item("codartcom").ToString.Substring(1) & "' and id_emp = '" & jytsistema.WorkID & "' ")
                    precioTotalF += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio from jsmercatser where codser = '" & .Item("codartcom").ToString.Substring(1) & "' and id_emp = '" & jytsistema.WorkID & "' ")
                Else
                    precioTotalA += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio_A from jsmerctainv where codart = '" & .Item("CODARTCOM") & "' and id_emp = '" & jytsistema.WorkID & "' ") / Equivalencia(myConn,  .Item("CODARTCOM"), .Item("UNIDAD"))
                    precioTotalB += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio_B from jsmerctainv where codart = '" & .Item("CODARTCOM") & "' and id_emp = '" & jytsistema.WorkID & "' ") / Equivalencia(myConn,  .Item("CODARTCOM"), .Item("UNIDAD"))
                    precioTotalC += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio_C from jsmerctainv where codart = '" & .Item("CODARTCOM") & "' and id_emp = '" & jytsistema.WorkID & "' ") / Equivalencia(myConn,  .Item("CODARTCOM"), .Item("UNIDAD"))
                    precioTotalD += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio_D from jsmerctainv where codart = '" & .Item("CODARTCOM") & "' and id_emp = '" & jytsistema.WorkID & "' ") / Equivalencia(myConn,  .Item("CODARTCOM"), .Item("UNIDAD"))
                    precioTotalE += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio_E from jsmerctainv where codart = '" & .Item("CODARTCOM") & "' and id_emp = '" & jytsistema.WorkID & "' ") / Equivalencia(myConn,  .Item("CODARTCOM"), .Item("UNIDAD"))
                    precioTotalF += .Item("cantidad") * ft.DevuelveScalarDoble(myConn, " select precio_F from jsmerctainv where codart = '" & .Item("CODARTCOM") & "' and id_emp = '" & jytsistema.WorkID & "' ") / Equivalencia(myConn,  .Item("CODARTCOM"), .Item("UNIDAD"))
                End If
            End With
        Next

        txtCostos.Text = ft.FormatoNumero(CostoTotal)
        txtPesos.Text = ft.FormatoNumero(PesoTotal)
    ...
    Private Sub IniciarGrilla()
        ...
        Dim aAnchos() As Integer = {100, 400, 90, 50, 100, 100}
