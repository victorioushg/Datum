...
Public Class scrMain
    ...
    Private Sub scrMain_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        'Dim oForm As Form
        '   For Each oForm In Me.MdiChildren
        ' MsgBox(oForm.Text & " " & oForm.Tag & " " & oForm.Name)
        ' Next
    End Sub
    ...
    Private Sub scrMain_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            myConn.Open()
            ds = DataSetRequery(ds, strSQLEmpresa, myConn, NombreTabla, lblInfo)
            dt = ds.Tables(NombreTabla)


            Dim nUser As String = EjecutarSTRSQL_Scalar(myConn, lblInfo, " SELECT USUARIO FROM jsconctausu where id_user = '" & jytsistema.sUsuario & "'  ")
            jytsistema.sFechadeTrabajo = Now()
            Me.Text += " " & jytsistema.nVersion & "  " & _
                IIf(nUser = "jytsuite", jytsistema.strConn.Split(";")(0), "")


            'If CStr(EjecutarSTRSQL_Scalar(myConn, lblInfo, "select usuario from jsconctausu where id_user = '" & jytsistema.sUsuario & "' ")) = "jytsuite" Then VerificaBaseDatos(myConn, jytsistema.WorkDataBase, lblInfo)
            ...
            If dt.Rows.Count > 0 Then
                With dt
                    ...
                    IniciarDiasTrabajo(myConn, ds, jytsistema.sFechadeTrabajo, lblInfo)
    ...
    Private Sub IniciarCierreDeRutas(MyConn As MySqlConnection, ds As DataSet)
        ...
        If CBool(ParametroPlus(MyConn, Gestion.iVentas, "VENFVEPA06")) Then  'SI PROCESO AUTOMATICO
            If Not ProcesoAutomaticoInicial(MyConn, lblInfo, ProcesoAutomatico.iCierreDiarioDeRutas, jytsistema.sFechadeTrabajo) Then

                Dim dtAsesores As DataTable
                Dim nTablaAsresor As String = "tblAsesores"

                ds = DataSetRequery(ds, " select * from jsvencatven where tipo = '" & TipoVendedor.iFuerzaventa & "' and id_emp = '" & jytsistema.WorkID & "' order by codven ", MyConn, nTablaAsresor, lblInfo)
                dtAsesores = ds.Tables(nTablaAsresor)
                ...
                For Each nRow As DataRow In dtAsesores.Rows
                    With nRow
                        ...
                        If CInt(EjecutarSTRSQL_ScalarPLUS(MyConn, " SELECT COUNT(*) FROM jsvenrutcie WHERE rutacerrada = '" & procesada & "' ")) = 0 Then _
                            EjecutarSTRSQL(MyConn, lblInfo, " INSERT INTO jsvenrutcie SET rutacerrada = '" & procesada & "' ")
        ...
        If LoginUser(myConn, lblInfo) <> "jytsuite" Then
            If CBool(ParametroPlus(MyConn, Gestion.iVentas, "VENPARAM24")) Then  'SI PROCESO AUTOMATICO
                If Not ProcesoAutomaticoInicial(myConn, lblInfo, ProcesoAutomatico.iBloqueoClientes, jytsistema.sFechadeTrabajo) Then
                    ...
                    Dim nTablaClientes As String = "tbl" & NumeroAleatorio(100000)
    ...
    Private Sub IniciarParametros_Y_Contadores()
        ...
        If Not qFound(myConn, lblInfo, "jsconctapar", bCampos, bStrings) Then
            EjecutarSTRSQL(myConn, lblInfo, "insert into jsconctapar set id_emp = '" & jytsistema.WorkID & "' ")
        End If
    ...
    Private Sub IniciarGestiones()
        ...
        'iPlusGases = 4 'iPlus -SIGME  +Cilindros
        'iPlusPrensa = 5
    ...
    Private Sub Timer1_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer2.Tick
        ...
        Dim aFldd() As String = {"id_emp"}
        Dim aStrr() As String = {jytsistema.WorkID}
        jytsistema.WorkName = qFoundAndSign(myConn, lblInfo, "jsconctaemp", aFldd, aStrr, "nombre")
        ...
        If activeChild Is Nothing Then
            ...
            tsLWin.Text = ""
        Else
            tsLNWin.Font = New Font("Arial", 8, FontStyle.Bold)
        ...
        If jytsistema.WorkExercise = "" Then
            ...
            tslEjercicio.Text = ""
        Else
            tslNEjercicio.Font = New Font("Arial", 8, FontStyle.Bold)
            ...
            tslEjercicio.Text = jytsistema.WorkExercise & " | " & FormatoFecha(CDate(qFoundAndSign(myConn, lblInfo, "jsconctaeje", aFld, aStr, "inicio").ToString)) & _
                        " | " & FormatoFecha(CDate(qFoundAndSign(myConn, lblInfo, "jsconctaeje", aFld, aStr, "cierre").ToString))
        End If
    ...
    Private Sub RibbonButton31_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RibbonButton31.Click
        ...
        Dim f As New jsBanProConciliacion
        If ChildFormOpen(Me, f) Then
            f.Activate()
        Else
            f.MdiParent = Me
    ...
    Private Sub RibbonButton65_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RibbonButton65.Click
        'REPORTES COMPRAS  - MOVIMIENTOS DE CXP
        Dim f As New jsComRepParametros
        f.Cargar(TipoCargaFormulario.iShowDialog, ReporteCompras.cMovimientosProveedores, "Movimientos de proveedores")
        f = Nothing
    End Sub
    ...
    Private Sub RibbonButton268_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RibbonButton268.Click
        ...
        If jytsistema.WorkBox <> "" Then
            Dim TipoImpresora As Integer = TipoImpresoraFiscal(myConn, lblInfo, jytsistema.WorkBox)
            EsperateUnPoquito(myConn)
            Select Case TipoImpresora
                Case 0
                    MensajeInformativo(lblInfo, "Imprime factura...")
                Case 1 ' Factura Fiscal preimpresa
                    MensajeInformativo(lblInfo, "Imprimienso Factura fiscal")
        ...
        Else
            MensajeCritico(lblInfo, "DEBE TENER CONFIGURADA UNA IMPRESORA FISCAL...")
        End If
        ...
        If jytsistema.WorkBox <> "" Then
            ...
            Select Case TipoImpresora
                Case 0
                    MensajeInformativo(lblInfo, "Imprime factura...")
                Case 1 ' Factura Fiscal preimpresa
                    MensajeInformativo(lblInfo, "IMPRIMIENDO Factura fiscal")
                Case 2, 5, 6 ' Impresora Tipo Aclas (TFHKAIF.DLL)
    ...
    Private Sub RibbonButton98_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RibbonButton98.Click
        'REPORTES(VENTAS - LIBRO IVAVENTAS)
        Dim f As New jsVenRepParametros
    ...
    Private Sub RibbonButton119_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RibbonButton119.Click
        ...
        f = Nothing
    End Sub
    'Private Sub RibbonButton120_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RibbonButton120.Click
    '    'REPORTES VENTAS  - ACTIVACION CLIENTES mes a mes
    '    Dim f As New jsSIGMERepParametros
    '    f.Cargar(TipoCargaFormulario.iShowDialog, ReporteMedicionGerencial.cActivacionClientesMesMes, "Activaciones Clientes Mes a Mes")
    '    f = Nothing
    'End Sub
    Private Sub RibbonButton121_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RibbonButton121.Click
        'REPORTES VENTAS  - VENTAS Y ACTIVACION DE CLIENTES
        Dim f As New jsVenRepParametrosDos
    ...
    Private Sub RibbonButton112_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RibbonButton112.Click
        'REPORTES VENTAS  - Ranking de Vetnas
        Dim f As New jsVenRepParametrosDos
    ...
    Private Sub RibbonButton137_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RibbonButton137.Click
        ...
        f = Nothing
    End Sub
    Private Sub RibbonButton138_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RibbonButton138.Click
    ...
    Private Sub RibbonButton190_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RibbonButton190.Click
        Dim f As New jsControlArcTablaSimple
        Dim nTblEmpresa As String = "tblEmpresa"
        Dim dtEmpresas As DataTable
        ds = DataSetRequery(ds, " select id_emp codigo, nombre descripcion from jsconctaemp order by id_emp ", myConn, nTblEmpresa, lblInfo)
        dtEmpresas = ds.Tables(nTblEmpresa)
        f.Cargar("Empresas", ds, dtEmpresas, nTblEmpresa, TipoCargaFormulario.iShowDialog, False)
        jytsistema.WorkID = f.Seleccion
        jytsistema.WorkExercise = ""
        f.Close()
        f = Nothing
    End Sub
    ...
    Private Sub RibbonButton_Click(sender As System.Object, e As System.EventArgs)
