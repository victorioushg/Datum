...
Public Class jsControlArcCorredoresCestaTicket
    ...
    Private strSQLIVA As String = SeleccionGENTablaIVA(jytsistema.sFechadeTrabajo)
    '" SELECT tipo, monto FROM jsconctaiva " _
    ...
    Private Sub jsControlArcCorredoresCestaTicket_FormClosed(sender As Object, e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        ft = Nothing
    End Sub
    ...
    Private Sub AsignaMov(ByVal nRow As Long, ByVal Actualiza As Boolean)
        ...
        If c >= 0 AndAlso dtValores.Rows.Count > 0 Then
            ...
            MostrarItemsEnMenuBarra(MenuBarra, c, dtValores.Rows.Count)
            dg.Refresh()
            dg.CurrentCell = dg(0, c)
        End If

        ft.ActivarMenuBarra(myConn, ds, dtValores, lRegion, MenuBarra, jytsistema.sUsuario)
    ...
    Private Sub AsignaTXT(ByVal nRow As Long)
        ...
        With dt
            ...
            With .Rows(nRow)
                ...
                txtLongitudBarra.Text = ft.FormatoEntero(.Item("lencodbar"))
                txtInicioPrecio.Text = ft.FormatoEntero(.Item("inicioprecio"))
                txtLongitudPrecio.Text = ft.FormatoEntero(.Item("lenprecio"))
                txtInicioTipo.Text = ft.FormatoEntero(.Item("iniciotipo"))
                txtLongitudTipo.Text = ft.FormatoEntero(.Item("lentipo"))
                txtCargos.Text = ft.FormatoNumero(.Item("cargos"))
                Proveedor = .Item("codpro")
                txtProveedor.Text = ft.DevuelveScalarCadena(myConn, " select nombre from jsprocatpro where codpro = '" & Proveedor & "' and id_emp = '" & jytsistema.WorkID & "' ")

                Grupo = .Item("grupo")
                SubGrupo = .Item("subgrupo")
                txtGrupoNombre.Text = ft.DevuelveScalarCadena(myConn, " select nombre from jsprogrugas where codigo = '" & Grupo & "' and id_emp = '" & jytsistema.WorkID & "' ")
                txtSubgrupoNombre.Text = ft.DevuelveScalarCadena(myConn, " select nombre from jsprogrugas where codigo = '" & SubGrupo & "' and id_emp = '" & jytsistema.WorkID & "' ")

                Dim aIVA() As String = ArregloIVA(myConn, lblInfo)
                Dim tIVA As Integer = ft.InArray(aIVA, .Item("tipoiva").ToString)
                ...
                txtNombre1.Text = ft.muestraCampoTexto(.Item("descrip"))
                ...
                Dim aAnc() As Integer = {50, 220, 60, 60}
    ...
    Private Sub AbrirMovimientos(ByVal CodigoCorredor As String)
        ...
        Dim aAnchos() As Integer = {75, 100, 300}
    ...
    Private Sub IniciarCorredor(ByVal Inicio As Boolean)
        ...
        If Inicio Then
            txtCodigo.Text = ft.autoCodigo(myConn, "codigo", "jsvencestic", "id_emp", jytsistema.WorkID, 5)
        Else
            txtCodigo.Text = ""
        End If
        ft.iniciarTextoObjetos(Transportables.tipoDato.Cadena, txtNombre, txtNombre1, txtProveedor, txtGrupoNombre, txtSubgrupoNombre, txtCodigoContable)
        ft.iniciarTextoObjetos(FormatoItemListView.iEntero, txtLongitudBarra, txtLongitudPrecio, txtLongitudTipo, txtInicioPrecio, txtInicioTipo)
        ft.iniciarTextoObjetos(Transportables.tipoDato.Numero, txtCargos)
    ...
    Private Sub ActivarMarco0()
        ...
        ft.habilitarObjetos(True, True, txtNombre, txtLongitudBarra, txtInicioPrecio, txtLongitudPrecio, txtInicioTipo, _
    ...
    Private Sub DesactivarMarco0()
        ...
        ft.habilitarObjetos(False, True, txtCodigo, txtCodigo1, txtNombre, txtNombre1, txtLongitudBarra, txtLongitudPrecio, txtLongitudTipo, _
    ...
    Private Sub EliminaCorredor()
        ...
        If ft.PreguntaEliminarRegistro() = Windows.Forms.DialogResult.Yes Then
            ...
            Else
                ft.mensajeCritico("Este CORREDOR posee movimientos. Verifique por favor ...")
            End If
        End If
    ...
    Private Function Validado() As Boolean
        ...
        If Proveedor = "" Then
            ft.mensajeAdvertencia("Debe indicar/seleccionar un proveedor válido")
            Return False
        End If

        If Grupo = "" Then
            ft.mensajeAdvertencia("Debe indicar un Grupo/Subgrupo válido...")
            Return False
        End If
    ...
    Private Sub txtCodigo_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCodigo.GotFocus, _
        ...
        Select Case sender.name
            Case "txtCodigo"
                ft.mensajeEtiqueta(lblInfo, " Indique el código corredor de cheques de alimentación ... ", Transportables.TipoMensaje.iInfo)
            Case "txtNombre"
                ft.mensajeEtiqueta(lblInfo, " Indique el nombre corredor de cheques de alimentacion ... ", Transportables.TipoMensaje.iInfo)
            Case "txtLongitudBarra"
                ft.mensajeEtiqueta(lblInfo, " Indique la longitud del código de barra del ticket ... ", Transportables.TipoMensaje.iInfo)
            Case "txtLongitudPrecio"
                ft.mensajeEtiqueta(lblInfo, " Indique la longitud de precio en el código de barra ... ", Transportables.TipoMensaje.iInfo)
            Case "txtLongitudTipo"
                ft.mensajeEtiqueta(lblInfo, " Indique la longitud del tipo en el código de barra  ... ", Transportables.TipoMensaje.iInfo)
            Case "txtInicioPrecio"
                ft.mensajeEtiqueta(lblInfo, " Indique la posición inicio del precio en el código de barras ... ", Transportables.TipoMensaje.iInfo)
            Case "txtInicioTipo"
                ft.mensajeEtiqueta(lblInfo, " Indique la posición inicio del tipo de ticket en el código de barras ... ", Transportables.TipoMensaje.iInfo)
            Case "txtCargos"
                ft.mensajeEtiqueta(lblInfo, " Indique el monto por cargos sobre la valija de tickets ... ", Transportables.TipoMensaje.iInfo)
            Case "btnProveedor"
                ft.mensajeEtiqueta(lblInfo, " Seleccione Proveedor asociado con el gasto(Cargo) de valija ... ", Transportables.TipoMensaje.iInfo)
            Case "btnGrupo"
                ft.mensajeEtiqueta(lblInfo, " Seleccione el grupo y subgrupo de gastos ... ", Transportables.TipoMensaje.iInfo)
            Case "btnCodigoContable"
                ft.mensajeEtiqueta(lblInfo, " Seleccione el código contable de este gasto ... ", Transportables.TipoMensaje.iInfo)
            Case "cmbIVA"
                ft.mensajeEtiqueta(lblInfo, " Seleccione el porcentaje de IVA para este gasto ... ", Transportables.TipoMensaje.iInfo)
    ...
    Private Sub dg_RowHeaderMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dg.RowHeaderMouseClick, _
        ...
        MostrarItemsEnMenuBarra(MenuBarra, e.RowIndex, ds.Tables(nTablaValores).Rows.Count)
    End Sub
    ...
    Private Sub btnGrupo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGrupo.Click
        ...
        If Grupo <> "" Then txtGrupoNombre.Text = ft.DevuelveScalarCadena(myConn, " select nombre from jsprogrugas where codigo = '" & Grupo & "' and id_emp = '" & jytsistema.WorkID & "' ")
        If SubGrupo <> "" Then txtSubgrupoNombre.Text = ft.DevuelveScalarCadena(myConn, " select nombre from jsprogrugas where codigo = '" & SubGrupo & "' and id_emp = '" & jytsistema.WorkID & "' ")
