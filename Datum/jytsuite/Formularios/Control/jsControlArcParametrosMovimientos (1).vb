...
Public Class jsControlArcParametrosMovimientos
    ...
    Private aGestion() As String = {"Contabilidad", "Bancos y Cajas", "Recursos Humanos", "Compras y cuentas por pagar", _
                                    "Ventas y cuentas por cobrar", "Puntos de Venta", "Mercancías", "Producción", "Medición Gerencial", "Control de Gestiones"}
    Private aTipo() As String = {"No/Si", "Doble", "Cadena", "Dia Semana", "Entero", "Cantidad o Peso", _
                                 "Fecha", "Formato de Papel", "Modo de Impresora", "Tipo de Impresora Fiscal", "Numeración Contable", _
                                 "Vector", "Tabla"}
    ...
    Public Sub Editar(ByVal MyCon As MySqlConnection, ByVal ds As DataSet, ByVal dt As DataTable)
        ...
        HabilitarObjetos(False, True, cmbGestion, txtCodigo, txtDescripcion, cmbTipo)
        Me.ShowDialog()
    ...
    Private Sub AsignarTXT(ByVal nPosicion As Integer)
        With dtLocal
            ...
            RellenaCombo(aTipo, cmbTipo, CInt(.Rows(nPosicion).Item("tipo")))
            RellenaCombo(aGestion, cmbGestion, .Rows(nPosicion).Item("gestion") - 1)
            VisualizarObjetos(False, MenuTabla, dgTabla)

            Select Case CInt(.Rows(nPosicion).Item("tipo"))
                Case TipoParametro.iNoSi
                    RellenaCombo(aNoSi, cmbValor, .Rows(nPosicion).Item("valor"))
                    VerTXT(False)
                Case TipoParametro.iDiaSemana
                    RellenaCombo(aDia, cmbValor, .Rows(nPosicion).Item("valor"))
                    VerTXT(False)
                Case TipoParametro.iFormatoPapel
                    RellenaCombo(aPapel, cmbValor, .Rows(nPosicion).Item("valor"))
                    VerTXT(False)
                Case TipoParametro.iModoImpresora
                    RellenaCombo(aModo, cmbValor, .Rows(nPosicion).Item("valor"))
                    VerTXT(False)
                Case TipoParametro.iTipoImpresoraFiscal
                    RellenaCombo(aFiscal, cmbValor, .Rows(nPosicion).Item("valor"))
                    VerTXT(False)
                Case TipoParametro.iNumeracionContable
                    RellenaCombo(aNum, cmbValor, .Rows(nPosicion).Item("valor"))
                    VerTXT(False)
                Case TipoParametro.iTabla
                    RellenaCombo(aTab, cmbValor)
                    VisualizarObjetos(True, MenuTabla, dgTabla)
                    HabilitarObjetos(False, True, txtValor)
                    IniciarGrilla()
                Case Else
    ...
    Private Sub txtCodigo_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCodigo.GotFocus
        MensajeEtiqueta(lblInfo, "Indique el código del parámetro ...", TipoMensaje.iInfo)
    End Sub
            ...
            Select Case cmbTipo.SelectedIndex
                ...
                Case Else
                    If txtValor.Text = "" Then
                        MensajeAdvertencia(lblInfo, "Debe indicar un valor al parámetro ...")
                        EnfocarTexto(txtValor)
                        Exit Function
                    End If
            End Select
        End If
    ...
    Private Sub btnOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOK.Click
        If Validado() Then
            ...
            End If
            InsertEditCONTROLParametro(MyConn, lblInfo, Insertar, cmbGestion.SelectedIndex + 1, nModulo, txtCodigo.Text, txtDescripcion.Text, cmbTipo.SelectedIndex, _
                IIf(txtValor.Visible, txtValor.Text, cmbValor.SelectedIndex))
    ...
    Private Sub btnAgregaDescuento_Click(sender As System.Object, e As System.EventArgs) Handles btnAgregaDescuento.Click
        Select Case txtCodigo.Text
            Case "VENPARAM38"
                EjecutarSTRSQL(MyConn, lblInfo, " replace into jsconnumcontrol " _
            ...
            Case "POSPARAM38"
                Dim aWhereFields() As String = {"modulo", "id_emp"}
                Dim aWhereValues() As String = {FormatoTablaSimple(Modulo.iFrasesCintillo), jytsistema.WorkID}
                EjecutarSTRSQL(MyConn, lblInfo, " replace into jsconctatab " _
                    & " set codigo = '" & AutoCodigoXPlus(MyConn, "codigo", "jsconctatab", aWhereFields, aWhereValues, 5, True) & "', " _
                    & " descrip = '', " _
            ...
            Case "POSPARAM46"
                Dim aWhereFields() As String = {"modulo", "id_emp"}
                Dim aWhereValues() As String = {FormatoTablaSimple(Modulo.iDenominacionesMonetarias), jytsistema.WorkID}
                EjecutarSTRSQL(MyConn, lblInfo, " replace into jsconctatab " _
                    & " set codigo = '" & AutoCodigoXPlus(MyConn, "codigo", "jsconctatab", aWhereFields, aWhereValues, 5, True) & "', " _
                    & " descrip = '', " _
            ...
            Case "VENPARAM38"
                If dgTabla.RowCount > 0 Then
                    If dgTabla.SelectedCells(0).RowIndex >= 0 Then
                        ...
                        EliminarRegistros(MyConn, lblInfo, dsLocal, nTablaTabla, "jsconnumcontrol", strSQL, aCamposDel, aStringsDel, _
                                                              nRow, True)
            ...
            Case "POSPARAM38"
                If dgTabla.RowCount > 0 Then
                    If dgTabla.SelectedCells(0).RowIndex >= 0 Then
                        ...
                        EliminarRegistros(MyConn, lblInfo, dsLocal, nTablaTabla, "jsconctatab", strSQL, aCamposDel, aStringsDel, _
                                                              nRow, True)
            ...
            Case "VENPARAM38"
                If dgTabla.CurrentCell.ColumnIndex = 1 Then
                    ...
                    If Not String.IsNullOrEmpty(numControl.ToString) Then
                        EjecutarSTRSQL(MyConn, lblInfo, " update jsconnumcontrol " _
            ...
            Case "POSPARAM38"
                If dgTabla.CurrentCell.ColumnIndex = 1 Then
                    If Not String.IsNullOrEmpty(dgTabla.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString) Then
                        ...
                        EjecutarSTRSQL(MyConn, lblInfo, " update jsconctatab " _
        ...
        Select Case txtCodigo.Text
                ...
                If Not String.IsNullOrEmpty(dgTabla.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString) Then
                    If e.ColumnIndex = 1 Then
                        If e.FormattedValue.ToString().Trim() = "" Then
                            MensajeAdvertencia(lblInfo, "DEBE INDICAR UN NUMERO DE CONTROL VALIDO...")
                                e.Cancel = True
                        End If
                    End If
                End If
            Case "POSPARAM38"
                If Not String.IsNullOrEmpty(dgTabla.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString) Then
                    If e.ColumnIndex = 1 Then
                        If e.FormattedValue.ToString().Trim() = "" Then
                            MensajeAdvertencia(lblInfo, "DEBE INDICAR UNA FRASE PARA CINTILLO VALIDA...")
                            e.Cancel = True
                        End If
                    End If
                End If
            Case "POSPARAM46"
                If Not String.IsNullOrEmpty(dgTabla.Rows(e.RowIndex).Cells(e.ColumnIndex).Value.ToString) Then
                    If e.ColumnIndex = 1 Then
                        If e.FormattedValue.ToString().Trim() = "" Then
                            MensajeAdvertencia(lblInfo, "DEBE INDICAR UNA DENNOMINACION MONETARIA VALIDA...")
                            e.Cancel = True
                        End If
                    End If
                End If
