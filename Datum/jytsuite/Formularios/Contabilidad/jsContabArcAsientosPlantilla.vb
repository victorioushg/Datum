...
Public Class jsContabArcAsientosPlantilla
    ...
    Private Sub jsContabArcAsientosPlantilla_FormClosed(sender As Object, e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        ft = Nothing
    End Sub
    ...
    Private Sub AsignaMov(ByVal nRow As Long, ByVal Actualiza As Boolean)
        ...
        If c >= 0 Then
            ...
            MostrarItemsEnMenuBarra(MenuBarraRenglon, c, dtMovimientos.Rows.Count)
            dg.Refresh()
            dg.CurrentCell = dg(0, c)
        End If
        ft.ActivarMenuBarra(myConn, ds, dtMovimientos, lRegion, MenuBarraRenglon, jytsistema.sUsuario)
    End Sub
    ...
    Private Sub AbrirMovimientos(ByVal NumeroAsiento As String, Optional nPosicionRenglon As Long = 0)
        ...
        Dim aAnchos() As Integer = {150, 200, 150, 400, 60, 40, 100}
    ...
    Private Sub ActivarMarco0()
        ...
        ft.habilitarObjetos(True, True, txtDescripcion)
    ...
    Private Sub DesactivarMarco0()
        ...
        ft.habilitarObjetos(False, True, txtAsiento, txtDescripcion)
    ...
    Private Sub GuardarTXT()
        ...
        If i_modo = movimiento.iAgregar Then
            ...
            numAsiento = ft.autoCodigo(myConn, "asiento", "jscotencdef", "id_emp", jytsistema.WorkID, 5)
        End If

        ft.Ejecutar_strSQL(myconn, " update jscotrendef set asiento = '" & numAsiento & "' " _
                            + " where " _
    ...
    Private Sub txtDescripcion_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtDescripcion.GotFocus
        ft.mensajeEtiqueta(lblInfo, " Indique el descripción para este asiento contable ... ", Transportables.TipoMensaje.iInfo)
        ...
        If LoginUser(myConn, lblInfo) = "jytsuite" Then
            If dt.Rows.Count > 0 Then
                ...
                If ft.PreguntaEliminarRegistro() = Windows.Forms.DialogResult.Yes Then
                    nPosicionEncab = EliminarRegistros(myConn, lblInfo, ds, nTabla, "jscotencdef", strSQL, aFld, aSFld, nPosicionEncab)
                    nPosicionRenglon = EliminarRegistros(myConn, lblInfo, ds, nTablaMovimientos, "jscotrendef", strSQLMov, aFld, aSFld, nPosicionRenglon)
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        ...
        Dim Anchos() As Integer = {100, 2500, 100}
        ...
        If nPosicionRenglon >= 0 Then
            ...
            If ft.PreguntaEliminarRegistro() = Windows.Forms.DialogResult.Yes Then
                With dtMovimientos.Rows(nPosicionRenglon)
                    ...
                    nPosicionRenglon = EliminarRegistros(myConn, lblInfo, ds, nTablaMovimientos, "jscotrendef", strSQLMov, aFld, aNFld, nPosicionRenglon)
                End With
            ...
            If dtMovimientos.Rows(0).Item("renglon") <> "" Then
                If dtMovimientos.Rows(nPosicionRenglon).Item("renglon") > dtMovimientos.Rows(0).Item("renglon") Then
                    ...
                    If qFound(myConn, lblInfo, "jscotrendef", afld, asFld) Then
                        ft.Ejecutar_strSQL(myconn, " update jscotrendef set renglon = 'XXXXX' where asiento = '" & txtAsiento.Text & "' and renglon = '" & RenglonDestino & "' and id_emp = '" & jytsistema.WorkID & "'  ")
                        ft.Ejecutar_strSQL(myconn, " update jscotrendef set renglon = '" & RenglonDestino & "' where asiento = '" & txtAsiento.Text & "' and renglon = '" & dtMovimientos.Rows(nPosicionRenglon).Item("renglon") & "' and id_emp = '" & jytsistema.WorkID & "'")
                        ft.Ejecutar_strSQL(myconn, " update jscotrendef set renglon = '" & dtMovimientos.Rows(nPosicionRenglon).Item("renglon") & "' where asiento = '" & txtAsiento.Text & "' and renglon = 'XXXXX' and id_emp = '" & jytsistema.WorkID & "'")
                    Else
                        ft.Ejecutar_strSQL(myconn, " update jscotrendef set renglon = '" & RenglonDestino & "' where asiento = '" & txtAsiento.Text & "' and renglon = '" & dtMovimientos.Rows(nPosicionRenglon).Item("renglon") & "' and id_emp = '" & jytsistema.WorkID & "'  ")
                    End If
                    nPosicionRenglon -= 1
