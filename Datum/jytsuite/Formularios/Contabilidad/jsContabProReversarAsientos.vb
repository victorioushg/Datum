...
Public Class jsContabProReversaraAsientos
    ...
    Public Sub Cargar(ByVal MyCon As MySqlConnection)
        ...
        ft.mensajeEtiqueta(lblInfo, "Seleccione asientos actuales ... ", Transportables.TipoMensaje.iAyuda)
    ...
    Private Sub ProcesarAsientos()
        ...
        For Each selectedItem As DataGridViewRow In dg.Rows
            With selectedItem
                refrescaBarraprogresoEtiqueta(ProgressBar1, lblProgreso, (iCont + 1) / dg.RowCount * 100, .Cells(1).Value.ToString)
                If CBool(selectedItem.Cells(0).Value) Then
                    documentos_procesados += .Cells(1).Value.ToString + " "
                    ft.Ejecutar_strSQL(myConn, " update jscotencasi set actual = " & Asiento.iDiferido & " where asiento = '" & .Cells(1).Value.ToString & "' and ejercicio = '" & jytsistema.WorkExercise & "' AND id_emp = '" & jytsistema.WorkID & "' ")
                    ft.Ejecutar_strSQL(myConn, " update jscotrenasi set actual = " & Asiento.iDiferido & " where asiento = '" & .Cells(1).Value.ToString & "' and ejercicio = '" & jytsistema.WorkExercise & "' AND id_emp = '" & jytsistema.WorkID & "' ")
